...
> Dernière mise à jour : 2026-06-24 — branche de travail : `claude/alliance-site-security-37fc6z` (tampon auto à chaque commit).
> **🆕 SESSION 24/06 (branche `claude/alliance-site-security-37fc6z`, PR #187) — OUTILS PENTEST :** (1) **`pentest-bridge/audit-generateur.html`** : générateur de rapport client autonome (1 fichier, hors-ligne, logo lion AG embarqué en base64). Formulaire + aperçu live, jauge SVG de note, failles par gravité, test d'intrusion OK/KO, devis de remédiation, **import « coller la synthèse Kali »** (parseur cible/profil/date/note + détection énumération comptes & fichiers exposés), **export PDF (impression)** et **export Markdown/texte (email)**. Exemple devsource.fr pré-rempli. (2) **`pentest-bridge/assistant-pentest.html`** : assistant de test d'intrusion GUIDÉ — on colle la sortie de chaque commande Kali (nmap/whatweb/wafw00f/wpscan/wp-json/testssl/feroxbuster/subfinder/nuclei), il alimente une **base de connaissances** (ports, CMS, WAF, comptes, plugins, .git, TLS…) + un **moteur de décision** qui propose les **prochaines étapes classées par priorité** (commande prête + pourquoi), accumule les failles et **copie une synthèse** prête pour le générateur. sqlmap/git-dumper/brute-force = marqués « actif, manuel, sous mandat ». (3) **Vérifié : `inc/ag-hardening.php` ferme bien `wp-json/users` (filtre `rest_endpoints`) ET `?author=`/archives auteur (`init` prio 1 + `template_redirect`)** — c'est LE correctif vendu (faille devsource.fr). ⚠️ Ces 2 HTML = outils de travail (non servis par le thème), à ouvrir dans un navigateur côté Fabrice. NB : la branche orpheline `claude/pentest-local-security-offers-KNehV` n'a PLUS d'ancêtre commun avec `main` (main reconstruit le 18/06) → ne pas tenter de la merger ; seul `scan.sh` défaut=max y est en plus (réappliquable à la main).
