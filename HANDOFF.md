...
> **⚠️ RESTE À ARMER CÔTÉ FABRICE** (tout est éteint par défaut) : Matteo (`ag_chasse_auto_on`), Hugo (`ag_closer_on`), Enzo (`ag_nego_on`), et surtout **le verrou `ag_sign_contrat_relu`** — rien ne part à signer tant que le modèle de contrat n'est pas coché comme relu par un professionnel du droit. **Décisions prises par Fabrice le 20/09 : contresignature AUTOMATIQUE et contrat envoyé AUTOMATIQUEMENT sur réponse positive** — avec ces deux-là, ce verrou est le dernier endroit où un humain regarde.
