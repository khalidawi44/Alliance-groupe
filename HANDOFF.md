...
> Dernière mise à jour : 2026-09-08 — branche de travail : `claude/pentest-local-security-offers-KNehV` (tampon auto à chaque commit).
> **🤖 SESSION 07/09 — ASSISTANT-DEFI = MODE ENCADRÉ + AUTO + RAPPORT 2 VERSIONS :** `assistant-defi.html` (2 copies sync : pentest-bridge/ + assets/tools/) a désormais : (1) **fix** : étapes débloquées ont leur zone « colle le résultat » (chaîne récursive) ; (2) **mode encadré** : encart « Charger un scope » (JSON in_scope/out_scope/tests_autorises/non_qualifying/rate_limit_rps, persiste localStorage) → `gen()` refuse hors-scope, saute les non-qualifying (anti-RTFS), throttle auto ; (3) **campagne auto** (`startCampaign`) : sélectionne les tests autorisés, génère, guide étape par étape (exécution réelle = Kali via `ag-runner.sh`, la page reste le cerveau) ; (4) **rapport présentable marque AG** (logo SVG inline) en 2 versions — NOVICE (client, langage clair, jauge) et EXPERT (CVSS/preuve/remédiation) + Imprimer/PDF. Findings accumulés (dedup). (5) **DEVIS de remédiation** dans le rapport client : prix par gravité éditables (Critique 900 / Élevé 500 / Moyen 250 / Faible 90 €, persistés) → tableau + total sur les 2 versions et le PDF, + **bouton « Lancer la remédiation & régler en ligne »** pointant vers un champ `payUrl` (le rapport/paiement caché sur le site). (6) **FIX loop wpscan** : analyser une sortie wpscan ne reboucle plus sur « WordPress détecté » (branche dédiée : comptes→test mdp, plugins→searchsploit). (7) **QR codes de paiement** (logo AG au centre, niveau H) sous chaque formule pour la version papier (lib `qrcode-generator` inlinée). (8) **FAIT côté site** : `inc/ag-rapport-client.php` = CPT « Rapports client » + page privée `/rapport/<jeton>` (non indexée) affichant le rapport + 3 formules avec vrais boutons de paiement Stripe/PayPal, `?offre=` surligne/scrolle. Le champ `payUrl` de l'assistant = cette URL. (9) **PONT KALI (exécution auto) FAIT** : bouton **« Exporter le plan Kali (.sh) »** → `campagne.sh` (bash, commandes scopées+throttlées, sleep=1/rps) ; sur Kali `bash campagne.sh` → `campagne-*.txt` ; section **« ③bis Importer les résultats »** → colle le .txt → découpe par `===== ... =====`, `scan()` chaque bloc, accumule les failles, construit le rapport auto. Boucle complète : export .sh → run Kali → recoller .txt → rapport. ⚠️ **À FAIRE ENSUITE** : (a) OPTIONNEL : version « zéro-recoller » via `ag-runner.sh` (file de jobs site↔Kali) si voulu ; (b) DÉPLOIEMENT de `ag-rapport-client.php` : nécessite SYNC GitHub (et selon la branche lue par le site, éventuellement merge dans `main`) + 1 visite Réglages→Permaliens si le rewrite ne prend pas ; créer les liens de paiement Stripe/PayPal par formule et les coller dans chaque Rapport client. Briefing tiers = `pentest-bridge/BRIEF-CHATGPT.md`. **Note Doctolib** : la prime 50 000 € = **preuve d'accès ADMIN** (démontrer un contrôle admin, pas un bug moyen) → viser une chaîne menant à l'admin. Durcissement client = `pentest-bridge/ag-hardening-muplugin.php` (Gwen `ag-gwen-services` PAS dans ce repo → à durcir dans la conv Alliance).
> **🧰 SESSION 17/08 — OUTILS HORS-LIGNE (pentest-bridge/) :** (1) `copilote-pentest.html` : fiche « réparer réseau/internet Kali » (mots-clés `réseau`/`internet`/`dns`/`proxy`) incluant le cas **proxy Firefox/Burp** (curl -I test → FoxyProxy OFF) — cause réelle du « pas d'internet » de Fabrice (DNS + proxy Burp, PAS de compromission ; wpscan agressif avait halluciné bloger/webshell : seul `blnmrpb` = 403 réel sur Hostinger, à vérifier hPanel un jour, non urgent). (2) **Générateur de rapport complet** ajouté dans `assistant-defi.html` ET `assistant-pentest.html` : format `MODELE-RAPPORT-YWH` éditable (titre + Bug type + Scope + Endpoint + Vulnerable part + 5 sections + tableau récap champs + CVSS), avec 2 types réels — **`bac`** (write non-auth + PREUVE DE CHANGEMENT façon titre SEO kalelia, CVSS 7.5) et **`enum`** (Doctolib, 5.3). (3) **FIX** : `</script>` littéral dans payloads XSS de `assistant-defi` cassait le rendu → `<\/script>`. (4) `maj-auto.bat` (racine) : auto-pull Windows planifiable (Fabrice n'avait pas les MAJ car son fichier local était en retard ; Claude n'a PAS accès à son PC/Chrome). Les 2 copies des outils (pentest-bridge/ + alliance-groupe-theme/assets/tools/) sont synchronisées.
> **🛡️ AUDIT CLIENT kalelia.fr (mandat signé) — RÉALISÉ (05/08), livrables au COFFRE (PAS dans le repo car vulns client) :** WordPress. Finding principal confirmé = **Rank Math SEO 1.0.267 → write NON authentifié** via `POST /wp-json/rankmath/v1/updateMeta` avec `{"objectID":0,"objectType":"post","meta":{...}}` (cause : `get_post_permissions_check` renvoie `true` si `objectID===0`, + pas de sanitisation dans `update_site_editor_homepage`) → **CVE-2025-12714 (modif SEO non-auth, 5.3)** + **CVE-2026-66702 (XSS stockée non-auth, 7.2 High)**. **Prouvé sur la prod** (HTTP 200 + `{"slug":"/","schemas":[]}`, reverté). Chaîne takeover = XSS non-auth → payload crée un admin via session/nonce de la victime → contrôle total. WP All Export Pro = **RCE potentielle (CVE-2024-7419)** mais version masquée (403/404) → à confirmer wp-admin/staging. Enable Media Replace 4.1.8 (XSS auth). mailin 3.3.3 = à jour. nuclei 0. Pas de chemin DB externe. Rapport technique = scratchpad `rapport-audit-kalelia.md` ; présentation client = artefact claude.ai. ⚠️ Labo Docker de repro du takeover = Rank Math ne bootait pas (bizarrerie env, pas la faille) → abandonné, la preuve prod+code suffit.
> **🎯 REPRISE DEMAIN — DOCTOLIB, chasse au High/Critical (BOLA données médicales) :** Objectif = viser la vraie prime (accès aux données médicales d'un AUTRE patient). CE QU'ON SAIT : les **documents** = `GET /v2/account/documents/<UUID>` (UUID aléatoire non énumérable + chemin scopé `/account/` → sans doute sécurisé, mais À TESTER en autorisation croisée). Le `creator_id` (account_id 89490609) fuite dans l'objet document. Master_patients = signed_id (blindé). **PLAN À REPRENDRE, méthode 2 comptes** : (1) créer/connecter **compte B** (2e numéro, ex. celui du fils) en 2e fenêtre privée ; (2) compte B → Mes documents → téléverser une image → noter l'**UUID_B** du document (`/v2/account/documents/<UUID_B>`) ; (3) depuis la session **compte A**, rejouer dans Burp Repeater `GET /v2/account/documents/<UUID_B>` → **200 + doc = 💥 BOLA High/Critical** (s'arrêter, masquer, rapporter format `pentest-bridge/MODELE-RAPPORT-YWH.md`) / **404 = 🔒 scopé, sécurisé → pivoter** vers d'autres objets à ID (rendez-vous, messagerie, ordonnances `shareable_prescription`, partages `sharings`). ⚠️ Piège vu : un test Burp qui « passe » peut être un faux signal si le format de la donnée injectée est faux (cf. mdp transformé côté client) → toujours confirmer le vrai comportement. Règle : 1 étape à la fois, attendre « ok ». 2 comptes = OK côté éthique (les 2 sont à Fabrice). Presse-papier Kali réparé (copier-coller dispo).
> **✅ BUG BOUNTY DOCTOLIB — SESSION 03-04/08 CLÔTURÉE (résultat) :** (1) IDOR sur « proches » = **confirmé BLINDÉ** : `DELETE /account/account_master_patients/<entier brut 177152942>` → **404** (Rails `signed_id`/HMAC obligatoire). Rien à rapporter. (2) **FINDING VALIDE trouvé + rapport soumis sur YWH** : **énumération d'utilisateurs NON authentifiée** via `POST /authn/patient/realms/doctolib-patient/accounts/check_existence` (host `www.doctolib.fr`, in-scope). Body `{"username":"<email|tél>","clientId":"patient-fr-client"}` → `{"account_exists":true/false}`. Testé sans session (fenêtre privée), `isCaptchaRequired:false`. **Rapport = https://yeswehack.com/reports/849656 → RÉSULTAT : REFUSÉ "RTFS" (non-qualifying, account enumeration hors périmètre Doctolib) le 06/08. -2 crédits non remboursés. LEÇON : lire la liste "non-qualifying / out of scope" AVANT de bosser un finding.** Fichier local = `pentest-bridge/rapport-doctolib-enum.md`. Form YWH : bug type **Improper Authentication (CWE-287)**, part **POST parameter/username**, impact **PII Leak**, **CVSS 5.3** (AV:N/AC:L/PR:N/UI:N/S:U/C:L/I:N/A:N). PoC vidéo (démo true/false + numéro perso réel → true). ⚖️ Attente possible « informative/by design » (énum au login), mais finding réel et propre = 1er vrai rapport YWH. (3) ❌ Cible **account_id d'autrui (200 €)** = PAS atteignable via login/signup (check_existence = booléen seul ; login = erreur générique ; account_id 89490609 seulement dans JWT/signed_id de SES propres objets). À retenter un autre jour = cartographie API dédiée (booking, messagerie, GraphQL). Compte test : ligne perso +33744829516, account_id 89490609, alias YWH `hakaliance-ywh-8542341d0a931ddb@yeswehack.ninja`.
> **✅ KALI VM — PRESSE-PAPIER RÉPARÉ (04/08) :** était cassé car Guest Additions 7.1.4 ne compilaient pas sur noyau Kali à jour `7.0.12+kali` + gcc-15. FIX APPLIQUÉ : (1) `sudo apt install linux-image-amd64 linux-headers-amd64 build-essential dkms` + reboot (noyau à jour + headers), (2) **maj VirtualBox hôte Windows en 7.2.x**, (3) réinsérer ISO Additions (devenu 7.2.14) → `sudo mount /dev/cdrom /mnt && sudo sh /mnt/VBoxLinuxAdditions.run` → build OK → `sudo reboot`. Presse-papier bidirectionnel Windows↔Kali fonctionne. (Refaire l'étape 3 si un futur update noyau recasse le module.)
> **🎯 ARCHIVE — recon initiale IDOR proches (20/07) :** Compte test créé (compte perso Fabrice, alias YWH `hakaliance-ywh-8542341d0a931ddb@yeswehack.ninja` SANS `+a` — le `+a` n'était pas forwardé), **account_id = 89490609**, ligne pro +33744829516. Burp OK (Intercept off, in-scope). **CE QU'ON A TROUVÉ** : le flux `/account/master_patients` (« Mes proches ») utilise des **Rails `signed_id`** (message+HMAC+purpose) partout → le champ `id` (`eyJfcmFpbHMi…--sig`) et l'endpoint `DELETE /account/account_master_patients/<signed_id>` sont **signés** = résistants à l'IDOR. Le `signed_id` du DELETE **emballe l'entier brut** `account_master_patient_id` (décodé base64 = 176386176). L'entier est **séquentiel global** (176386176 → 176390693 en ~1h) DONC énumérable **SI** une route accepte l'entier nu. **DERNIÈRE ÉTAPE LANCÉE, RÉPONSE PAS ENCORE REÇUE (= reprendre ICI)** : test décisif dans Burp **Repeater** → prendre la requête `DELETE /account/account_master_patients/eyJ…`, remplacer le signed_id par l'entier brut **`176390693`** (un proche test qui EXISTE), Send, lire le code : **200/204/302 → 🎯 IDOR confirmé** (s'arrêter, ne toucher AUCUN autre patient, rédiger rapport YWH) ; **404/400/401/422 → 🔒 blindé**, PIVOTER vers la cible **« retrouver l'account_id d'autrui via email/téléphone » (prime fixe 200 €)** = énumération passive d'utilisateur. ⚠️ Cookies du DELETE capturé peut-être un peu vieux → si code d'auth bizarre (401/403 inattendu), recapturer une requête fraîche `www.doctolib.fr` et refaire le test. Règle : 1 étape à la fois, attendre « ok ». Format rapport = `pentest-bridge/MODELE-RAPPORT-YWH.md`.
