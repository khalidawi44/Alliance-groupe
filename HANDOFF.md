...
> Dernière mise à jour : 2026-06-01 — branche de travail : `claude/pentest-local-security-offers-KNehV` (tampon auto à chaque commit).
> **🔖 À REPRENDRE DEMAIN (pause 01/06 ~01h45)** : (1) **SORTIR LES RAPPORTS KALI** — ils sont dans la VM Kali (`~/pentest-local/rapports/<site>-<date>/` : 01-whatweb…07-wpscan + SYNTHESE.md), PAS sur la clé D:\pentest-alliance (Additions invité VirtualBox cassées → pas de partage auto). Méthode validée : zipper côté Kali (`cd ~/pentest-local/rapports && zip -r site.zip <dossier>` ou `tar -czf`) puis récupérer via Firefox-Kali / mini-serveur. ⚠️ **Kali a planté ce soir** → au redémarrage, vérifier que les dossiers rapports sont toujours là (`ls ~/pentest-local/rapports/`). Notepad refusait d'enregistrer car le nom = URL avec `:` et `/` (interdits Windows) → nommer `rapport-<client>.txt`. (2) **Clé D: VeraCrypt ?** à confirmer (ouvrir VeraCrypt, voir si D: monté) — sinon chiffrer (rapports = données clients sensibles). (3) **3 audits faits** : alliancegroupe-inc (sain), webmaster-gironde (WAF Anubis, sain), kristinaamarandos (2 points mineurs : X-Content-Type-Options absent + BREACH/deflate ; rien de critique). Rapport client + devis kristinaamarandos déjà rédigés dans le chat (barème : 90€ rapport / 250€ pack durcissement / 49€/mois maintenance). (4) **TODO site** : afficher le barème prix sur page offres sécurité + (optionnel validé) générateur PDF du rapport depuis la synthèse Kali dans la fiche client.
