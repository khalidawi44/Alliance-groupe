...
> **✅ FAIT (22/09) — GWEN SERVICES : RÉSERVATION EN LIGNE + AGENDA + ESPACE CLIENT, DE BOUT EN BOUT.** ⚠️ **Gwen a un site SÉPARÉ** : `gwen-services.alliancegroupe-inc.com`, thème **vendu `ag-gwen-services`** (PAS `alliance-groupe-theme`). Toute évolution passe donc par `assets/downloads/ag-gwen-services/` + `bash scripts/release.sh ag-gwen-services <version>` (auto-update du site client). **Version publiée : 1.3.3.** Modules ajoutés dans ce thème : `inc/ag-reservation.php` (réservation temps réel sur le planning de Gwen — après-midis = interventions ; invitations `.ics` client+Gwen ; flux d'abonnement privé `/wp-json/ag/v1/gwen.ics?token=` que Gwen abonne UNE fois ; espace client **autonome** — inscription/connexion en façade + `/mon-espace` + `/reserver` auto-créées ; menu : « Réserver » + liens compte dynamiques ; écran admin **📅 Réservations**) et `inc/ag-smtp.php` (**SMTP authentifié porté depuis le thème agence** — sinon les emails ne partent pas : `mail()` bridé Hostinger). **Délivrabilité résolue = 9,5/10 mail-tester** en authentifiant Gwen's SMTP avec le mailbox du domaine principal **`contact@alliancegroupe-inc.com`** (déjà SPF+DKIM+DMARC) ; **le piège était le mot de passe du mailbox** (souvent en `AG_SMTP_PASS` de wp-config côté agence, donc inconnu → à réinitialiser dans hPanel). Réglages → ✉️ Envoi des e-mails, case « authentifier » cochée. **Visuel = lane Cowork** (j'ai laissé des styles neutres). Alternative « 100 % marque Gwen » non faite : mailbox sur le sous-domaine gwen-services + SPF/DKIM/DMARC dédiés + chauffe.
> **✅ FAIT (21/09 soir) — LA CHAÎNE COMMERCIALE EST ARMÉE DE BOUT EN BOUT.** Le verrou avocat est levé (`ag_sign_contrat_relu` coché par Fabrice après relecture) → Camille + Margot actifs, la chaîne va du prospect repéré au contrat scellé. **Signature simplifiée** (`inc/ag-signature.php`) : le client ne tape plus son nom (affiché, déjà connu de l'affaire), il coche **UNE case « j'accepte et je signe »** ; le **code email à usage unique reste** (étape 2) — c'est l'Article 6 relu par l'avocat, la valeur juridique. **Hugo durci** (`inc/ag-closer.php`) : ne démarche JAMAIS les adresses de la maison (`ag_closer_email_bloque()` + champ « Ne jamais démarcher » dans l'écran Hugo), et **branche les 2 outils maison sur le site DU prospect** — maquette (`/refais-mon-site?url=`, prérempli+auto) au 1er mail, audit (`/audit-seo?url=`, formulaire prérempli = capture lead) au 3e mail ; un seul lien fort par mail (délivrabilité). Dossier de relecture avocat = `docs/RELECTURE-CONTRAT.md`. **⚠️ SEUL RISQUE OPÉRATIONNEL RESTANT : la boîte `contact@` doit VRAIMENT recevoir les réponses.** Alessia la relève en IMAP ; si la redirection vers Gmail ne **conserve pas de copie**, la boîte se vide, Alessia ne lit rien → Enzo et le contrat ne partent JAMAIS, en silence. À vérifier dans hPanel (redirection = conserver une copie). **Chauffe du domaine : plafond Hugo à 5/jour la 1re semaine.**
...
> **✅ FAIT (20/09) — LE SITE NE POUVAIT PAS ENVOYER D'E-MAIL, ET RIEN NE LE DISAIT.** WordPress remettait ses messages à la fonction `mail()` de PHP, que Hostinger bride : `wp_mail()` répondait « vrai », le message ne partait jamais. **Conséquence longtemps invisible : personne ne pouvait signer un contrat** (le code à usage unique n'arrivait pas), l'exemplaire signé n'arrivait ni au client ni à la maison, et les alertes devis/leads disparaissaient — masqué parce que les alertes passent AUSSI par SMS et Telegram, qui marchent. Corrigé par **`inc/ag-smtp.php`** (écran **✉️ Envoi des e-mails**) : sortie authentifiée via `smtp.hostinger.com:465` SSL, expéditeur aligné sur l'identifiant, et **plus aucun échec silencieux** (`wp_mail_failed` est capturé et affiché en clair). Mot de passe en base, sauf si `AG_SMTP_PASS` est défini dans `wp-config.php` — la constante l'emporte. **Vérifié en vrai** : message reçu en boîte de réception Gmail, « signé par alliancegroupe-inc.com ». DKIM publié côté DNS (3 CNAME `hostingermail-a/b/c._domainkey`). **⚠️ RESTE : DMARC est en `p=none`** — il observe et ne bloque rien. ⚠️ Piège rencontré deux fois : une **redirection n'est pas une boîte mail** (pas de mot de passe → `535`), et mon contrôle avant vol ne cherchait le DKIM qu'en TXT alors que Hostinger le publie en **CNAME** — il affichait vert sur un domaine non signé (corrigé).
> **✅ FAIT (21/09) — LE CABINET COMMERCIAL EST RELIÉ D'UN BOUT À L'AUTRE.** Organigramme dans `inc/ag-equipe.php` (écran **👥 Le cabinet**) : Naples/Nantes, un prénom par poste — Matteo (prospection), Hugo (démarchage, `ag-closer`), Alessia (réponses, `ag-reponses`), **Enzo (négociation, `ag-negociateur.php`, NOUVEAU)**, Giulia (offre), Camille (juridique, `ag-juriste`), Margot (signature, `ag-signature`), Salvo (coach). **Ces prénoms sont INTERNES** — les courriels partent signés « Alliance Groupe » avec la mention d'automatisation ; ne jamais les faire signer par un collaborateur fictif. ⚠️ Le chat du site utilise d'AUTRES prénoms (Léo/Sofia/Karim/Nadia) : arbitrage laissé à la lane DESIGN, consigné dans `COWORK.md`.
> **Le maillon qui manquait était l'ÉCOUTE** : Alessia attendait qu'on lui pousse les réponses sur `/wp-json/ag/v1/reponse`, que rien n'alimentait — donc ni qualification, ni négociation, ni contrat. **`inc/ag-boite.php`** relève désormais la boîte `contact@` en IMAP toutes les 10 min (`imap.hostinger.com:993`, identifiants repris de l'envoi). **Vérifié en vrai le 21/09 : 28 messages dans la boîte, 1 dans la fenêtre, 1 transmis à Alessia.** Garanties tenues par le code, pas par l'intention : connexion **OP_READONLY** (rien n'est marqué lu/déplacé/supprimé), **seuls les expéditeurs présents dans `ag_prospects`** sont lus et transmis, fenêtre 3 jours, 20 messages max par relève, dédoublonnage par Message-ID stocké côté site.
> **Garde-fous d'Enzo éprouvés** (8 cas, tous passent) : promesse de position Google, « garanti », « remise », mensualité sous le plancher, chiffre inventé, message vide → **BLOQUÉS** ; réponse honnête avec geste en nature → passe. Liste des promesses interdites **commune** avec le juriste (`ag_promesses_interdites()`, une seule source). Un message bloqué n'est pas envoyé : alerte + journal.
> **✅ FAIT (21/09) — CHAÎNE DE MONTAGE PASSIVE DES FICHES (`inc/ag-enrichir.php`, écran « 🔎 Compléter les fiches »).** Google Places ne rend JAMAIS d'email → 81 fiches/96 hors de portée de Hugo. La nuit (3h, 20 fiches, priorité aux « site sans email »), le site RELIT ce que le site du prospect publie — **aucun appel Google, aucune intrusion, uniquement du passif** — via `ag_audit_extract_contacts()` (email/SIRET) et `ag_audit_get()` (note). Choix d'email : fonction sur le domaine > domaine > gratuit. **LIGNE ROUGE tenue par le code :** le constat donné à Hugo passe par LISTE BLANCHE (`ag_enrich_constat`) — seuls les défauts VISIBLES DE DEHORS et NON exploitables deviennent une phrase (pas de HTTPS, pas mobile, lent, titre Google manquant). **Les failles exploitables (.git, versions, énumération, xmlrpc) restent MUETTES** et attendent le rapport d'audit remis à un client qui a SIGNÉ une autorisation. Testé hors WP : un site plein de failles ne rend qu'un constat de surface, zéro fuite. Le constat rejoint Hugo par le filtre `ag_closer_contexte_extra`. **Éteint par défaut** (`ag_enrich_on`=0). **⚠️ DÉCISION FABRICE (21/09) : les rapports Kali/scan ACTIF ne sont JAMAIS envoyés à un prospect froid** (interdit art. 323-1, + menace déguisée) — le Kali est la prestation vendue APRÈS autorisation signée, jamais l'hameçon.
> **✅ FAIT (21/09) — RATTRAPAGE DU CHAUD + PROVENANCE + MONTÉE EN GAMME.** Suite à l'analyse du tunnel (2 sous-agents + vérifs code) : « le froid est industrialisé, le chaud meurt en silence ». Construits, testés hors-WP, poussés, **tous éteints/neutres par défaut** :
> · **`ag-relance-chaud.php`** (écran 🔥 Relance du chaud) : relance contrat-envoyé-non-signé, intéressé silencieux, devis sans suite — e-mail + SMS si mobile. Opt-out respecté, 2 relances max (J+2/J+5). Ajouté au Directeur. **VÉRIFIÉ EN PROD le 21/09 : 1 intéressé relancé.**
> · **`ag-captures.php`** : handler du formulaire Contact (rendait page blanche sans WPForms), `ag_prospect_upsert()` qui ENRICHIT au lieu de jeter un lead chaud doublon, injection des demandes sur-mesure au CRM (`do_action('ag_sur_mesure_saved')` dans functions.php).
> · **`ag-ia.php`** : plafond de dépense IA (`ag_ia_cap_jour`, défaut 300, 0=illimité) — le seul coût autonome qui n'avait aucun frein.
> · **`ag-surclassement.php`** : montée en gamme par LIEN clair (maquette 9,90→Essentiel→Pro→Boutique→sur-mesure), prix affiché, **jamais de case pré-cochée ni contrat caché** (choix de Fabrice + droit conso). Glissé dans la relance des intéressés ET l'e-mail de bienvenue client.
> · **`ag-provenance.php`** (écran 🧭 Origine des comptes) : capte à la 1re visite le canal (Google/FB/lien ambassadeur ?ref=/parrain/UTM/référent/page d'arrivée) en first-touch, relie la création de compte au parcours (user_meta `ag_prov`), résumé sur la fiche membre. **Alerte push+journal à chaque nouveau compte.** RGPD : capte seulement si cookie de mesure présent (consentement). Comptes créés AVANT = « origine inconnue » (on ne réécrit pas le passé).
> **⚠️ Harmonie e-mails** : le gabarit `ag_email_wrap` (logo, en-tête or, pied légal) est la charte commune ; un rapport client qui partait en texte brut a été rewrappé. **Reste (lane DESIGN, dans COWORK.md) : le rendu FLOUTÉ de /refais-mon-site contredit sa promesse « sans inscription » — choix visuel à trancher.**
> **✅ FAIT (21/09) — HUGO CHOISIT L'ANGLE (ag-closer.php).** On n'exclut PAS les pros du web. `ag_closer_est_pro_web()` détecte métier/nom (webmaster, agence web, dev, SEO, wordpress… termes COMPOSÉS, « web » seul ne matche pas → pas de faux positif type « Le Web Café »). `ag_closer_redige()` calcule l'angle : pro web → **SÉCURITÉ** (auditer les sites de SES clients, d'égal à égal, jamais lui créer un site) ; pas de vrai site → CRÉATION ; site existant → AMÉLIORATION selon le constat. **Vérifié en aperçu prod (21/09) : webmaster → objet « Audit de sécurité pour vos clients », ton confrère, surface only.** Ligne rouge maintenue même en angle sécurité : jamais de liste de failles exploitables (constat de surface uniquement). Liste des métiers web filtrable via `ag_closer_metiers_web`.
> **✅ RÉSOLU DÉFINITIVEMENT (21/09) — DÉLIVRABILITÉ E-MAIL.** Symptôme : les tests n'arrivaient pas / tombaient en spam. CAUSE : le champ « Adresse d'envoi » du closer contenait un sous-domaine `contact@pro.alliancegroupe-inc.com` inexistant → From non signable → Gmail quarantaine, et erreur PHPMailer « Adresse invalide (From) ». CORRECTIF (`ag-smtp.php`, commit 71b3cc2) : `phpmailer_init` FORCE désormais l'expéditeur sur le domaine de l'identifiant SMTP — toute adresse hors domaine est ignorée, `setFrom(...,true)` écrase tout From posé en amont (closer). **VÉRIFIÉ EN PROD 21/09 14:29 : message reçu en BOÎTE DE RÉCEPTION (pas spam), De contact@alliancegroupe-inc.com, « signé par alliancegroupe-inc.com ».** SPF+DKIM+DMARC(p=quarantine) alignés. ⚠️ Un futur sous-domaine dédié `pro.` reste possible mais devra avoir SA PROPRE boîte + SPF + DKIM avant usage. ⚠️ Les rapports DMARC (rua) de google arrivent en spam chez contact@ : normal (XML zippés), ne rien en conclure.
> **🔒 DÉCISIONS DE FABRICE (21/09) — NE PAS LES DÉFAIRE.** (1) **Le classement de démonstration RESTE ACTIF** (`ag_demo_leaderboard_on` = 1, 4 faux ambassadeurs + 6 faux recruteurs). C'est un choix assumé d'amorçage, pas un oubli : aucune session ne doit le couper de sa propre initiative. (2) **Hugo (démarchage) a été RÉÉTEINT** le 21/09 dans la foulée — ne pas le rallumer sans le lui demander.
> **⚠️ ÉTAT AU 21/09 À MIDI : Hugo (démarchage) a été ALLUMÉ par Fabrice** — 14 prospects joignables, plafond 20/jour, 0 envoyé au moment où il l'a coché. **De vrais messages partent donc vers de vraies entreprises.** Conseil donné : descendre le plafond à 5 les deux premiers jours (un domaine qui n'a jamais démarché et part à 20 d'emblée se fait classer en indésirable — et les contrats partent du même domaine). ⚠️ Le contrôle avant vol affiche DKIM « sélecteur `hostingermail1` » : c'est l'ANCIENNE clé, encore publiée à côté des trois `hostingermail-a/b/c`. La signature marche (vérifiée chez Gmail), mais cette vieille clé brouille le diagnostic — à supprimer de la zone DNS au prochain passage.
> **⚠️ RESTE À ARMER CÔTÉ FABRICE** (le reste est éteint par défaut) : Matteo (`ag_chasse_auto_on`), Hugo (`ag_closer_on`), Enzo (`ag_nego_on`), et surtout **le verrou `ag_sign_contrat_relu`** — rien ne part à signer tant que le modèle de contrat n'est pas coché comme relu par un professionnel du droit. **Décisions prises par Fabrice le 20/09 : contresignature AUTOMATIQUE et contrat envoyé AUTOMATIQUEMENT sur réponse positive** — avec ces deux-là, ce verrou est le dernier endroit où un humain regarde.
...
> Dernière mise à jour : 2026-09-24 — branche de travail : `claude/network-access-params-mwh0me` (tampon auto à chaque commit).
