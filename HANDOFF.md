...
> Dernière mise à jour : 2026-06-23 — branche de travail : `claude/pentest-local-security-offers-KNehV` (tampon auto à chaque commit).
...
> **🔐 EN COURS — PENTEST AUTHENTIFIÉ devsource.fr (WordPress, MANDAT signé + identifiants fournis par le client)** : le client (agence Web&IA, « connaît pas la sécu ») a donné ses **identifiants de connexion** et demandé par écrit de **tester l'injection SQL** ET la **robustesse des mots de passe**. À FAIRE : (1) `wpscan -e u` (énum users) + attaque dico douce (`--passwords rockyou --password-attack wp-login --throttle 800 --max-threads 1`) ; (2) `wpscan -e vp,vt --plugins-detection aggressive` (plugins/thèmes vulnérables = 1re source SQLi) ; (3) `sqlmap` authentifié via cookie de session (`--crawl=2 --forms --level=2 --risk=1 --delay=1 --threads=1`). ⚠️ **Les 2 commandes wpscan/sqlmap données le 23/06 ont échoué → DIAGNOSTIQUER (erreur exacte à récupérer : outil pas installé ? wordlist absente ? DNS NordVPN ?).** Garde-fous : prod → throttle, risk=1 jamais 3, prévenir le client de la fenêtre (brute-force peut le lock). Profil scan auto = max (scan.sh défaut=max désormais). **À RAPPELER À FABRICE tant que le test mdp+SQL n'est pas fait + rapport client rédigé.**
> **🔒 RÈGLE PERMANENTE (demande Fabrice) — APRÈS CHAQUE SCAN, TOUJOURS LE RAPPELER (procédure VALIDÉE 23/06)** : (1) mettre le rapport dans le **coffre VeraCrypt** ; (2) **EFFACER les rapports de Kali** (aucune donnée client en clair ne doit rester dans la VM). **COMMANDE EXACTE** (depuis `/root/pentest-local`) : `sudo env AG_SHARE=/media/sf_7_download_kali ./coffre.sh` — ⚠️ il FAUT `env` (sinon sudo efface la variable `AG_SHARE` → erreur « dossier partagé absent »). Si `coffre.sh: command not found` ou `No such file` → le script n'est pas dans la VM : `curl -fsSL "https://raw.githubusercontent.com/khalidawi44/alliance-groupe/claude/pentest-local-security-offers-KNehV/pentest-bridge/coffre.sh" -o coffre.sh && chmod +x coffre.sh`. Si le curl échoue (DNS NordVPN) → **fallback manuel** : `base="<dossier-rapport>"; zip -r "/media/sf_7_download_kali/$base.zip" "./rapports/$base/" && find "./rapports/$base" -type f -exec shred -u -n 1 {} \; ; rm -rf "./rapports/$base" /usr/share/cmseek/Result/* /tmp/ag-* ; : > ~/.zsh_history ; : > ~/.bash_history`. Puis Windows : Mount VeraCrypt → déplacer le zip dans le coffre → Dismount → vider la corbeille. À redire à Fabrice à la fin de CHAQUE audit.
> **⚙️ PRÉFÉRENCE FABRICE — SCANS KALI TOUJOURS EN PROFIL `max`** (le plus exhaustif) par défaut, sauf demande contraire.
> **📐 RÈGLE DE MÉTHODE AUDIT (demande Fabrice 23/06) — ORDRE FIXE par cible** : (1) **test approfondi** (Espace Audit du site), puis (2) **scan Kali max**, puis (3) **test d'entrée / intrusion** (comme devsource : énum users + xmlrpc + login caché + fingerprint versions + spray léger si login trouvé). **La note d'un test plus poussé ne peut JAMAIS dépasser celle d'un test plus léger** : elle part du min des notes des modes plus légers du même site et ne fait que dévaluer (implémenté dans `ag_audit_hist_upsert`, rangs passive<deep<expert, plafond auto par hôte). **PAS toujours WordPress** → en intrusion, enchaîner les commandes **1 par 1** (DNS check → fingerprint → login → spray ciblé), s'adapter au CMS/techno réel, jamais un bloc auto. Rester non destructif + mandat signé.
> **🔐 BILAN devsource.fr (23/06, mandat test d'intrusion)** : WordPress chez o2switch. **Intrusion ÉCHOUÉE = bonnes défenses** : login caché (wps-hide-login, slug introuvable, /wp-admin→/erreur/), **xmlrpc 403**, limit-login actif, **Elementor 4.0.0 à jour** (pas de RCE). **1 faille réelle confirmée : énumération des comptes via wp-json** (edouard, maximeguinard, maximeguinard1). Reco : fermer l'énum REST + maintenance MAJ. Synthèse « 1 bis » rédigée (note plafonnée auto par la règle ci-dessus). Rapport Kali matinal `devsource.fr-20260623-061225` → **à mettre au coffre** (pas encore confirmé).
