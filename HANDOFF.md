...
> **⏰ RAPPEL À DIRE À FABRICE DÈS RECONNEXION :** finir le **paiement + déblocage AUTO du rapport client** (décisions prises : compte créé APRÈS paiement ; paiement = moyen déjà configuré pour les produits). À coder : checkout email sur le rapport → commande en attente (`ag_audit_pending`) → hook `ag_paypal_payment_verified` → créer compte `ag_client` + débloquer rapport complet + email des accès. PRÉVOIR un **test de paiement réel de 49 €**.
> Dernière mise à jour : 2026-07-22 — branche de travail : `claude/network-access-params-mwh0me` (tampon auto à chaque commit).
