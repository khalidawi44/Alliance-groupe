...
> **🔧 KALI VM — PRESSE-PAPIER Windows↔Kali CASSÉ (04/08, à réparer plus tard) :** module `vboxguest` absent (`/dev/vboxguest` No such file). Cause : Guest Additions **7.1.4** (VirtualBox hôte) **ne compilent pas** sur le noyau Kali à jour `7.0.12+kali` + **gcc-15** (erreurs `the-linux-kernel.h` / `iprt/types.h` No such file → build Error 2). Décalage de versions (paquet apt guest = 7.2.8, ISO = 7.1.4). **FIX = mettre à jour VirtualBox côté Windows en 7.2.x** (Aide → maj, ou virtualbox.org) puis réinsérer l'ISO des Additions + `sudo sh /mnt/VBoxLinuxAdditions.run` + reboot. Noyau à jour + headers déjà OK (`linux-headers-amd64` installés). En attendant : copier-coller INTERNE à Kali (Firefox↔Burp) marche → on chasse en mode screenshot, aucun blocage.
