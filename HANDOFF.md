...
> Dernière mise à jour : 2026-06-23 — branche de travail : `claude/conversation-linking-bug-O6HUR` (tampon auto à chaque commit).
> **🎨 NOUVEAU (23/06) — GÉNÉRATION DE VISUELS / VIDÉOS DANS LE CONTENEUR (capacités réelles à réutiliser)** :
>   - **Le conteneur distant sait fabriquer des IMAGES lui-même** (sans Canva/Picsart) : **Pillow** + **qrcode** (`pip install qrcode pillow`) + **Playwright/Chromium** déjà installés (binaire `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`, lancer avec `NODE_PATH=/opt/node22/lib/node_modules node script.cjs`, args `--no-sandbox --ignore-certificate-errors`, `ignoreHTTPSErrors:true`). Méthode qui marche le mieux = **rendre du HTML/CSS/SVG via Chromium puis screenshot** (`page.locator('.card').screenshot`), bien meilleur que Pillow pur. Techniques validées pour le style **« AG Starter PRO » noir & or** : texte or extrudé 3D (empilement de `text-shadow` dégradés + `-webkit-background-clip:text`), filtres SVG `feSpecularLighting`/`feDropShadow` pour le relief, volutes en `<path>` SVG, gemme facettée en polygone, QR base64. Scripts de réf créés en /tmp (lux2/render3.cjs = portrait façon carte PRO). → On peut donc générer/itérer des cartes/posts/flyers directement et les envoyer (SendUserFile).
>   - **POLICE** : Playfair Display vient de **Google Fonts** → BLOQUÉE tant que `fonts.googleapis.com` n'est pas dans l'allowlist réseau (sinon repli Liberation Serif). En local seules les polices **Liberation** (Serif/Sans) sont dispo.
>   - **VIDÉO** : (1) voie officielle = **Remotion** dans le repo séparé `alliance-videos` (cf `CLAUDE.md §4` : `video-remotion/AGShort.tsx`+`scripts.ts`+`Root.tsx`) ; (2) voie conteneur possible = générer des **frames** (Chromium/Pillow) puis assembler avec **ffmpeg** si présent (à vérifier `which ffmpeg`).
>   - **MCP Canva/Picsart = bloqués sur l'ANCIEN environnement** (réseau « Trusted » → egress vers canva/picsart/google refusé ; symptômes : Canva « flotte/déconnecté », Picsart « requires approval », fetch = « Host not in allowlist »). **CAUSE RACINE = la politique réseau de l'environnement, pas un bug.**
> **🌐 NOUVEL ENVIRONNEMENT « Custom » créé (23/06) pour débloquer tout** : claude.ai/code → env « Atlas groupe » → ⚙️ → **Accès réseau = Personnalisé** + cocher « liste par défaut des gestionnaires de paquets » + **Domaines autorisés** : `*.canva.com canva.com canva.link *.canva.ai *.canva-apps.com *.picsart.com *.picsart.io fonts.googleapis.com fonts.gstatic.com *.googleapis.com accounts.google.com *.github.com raw.githubusercontent.com *.wordpress.org *.notion.com *.notion.so api.calendly.com *.ifttt.com *.slack.com images.unsplash.com cdn.jsdelivr.net unpkg.com alliancegroupe-inc.com *.alliancegroupe-inc.com`. Setup script : `pip install --quiet qrcode pillow`. ⚠️ **Le changement ne prend QUE dans une NOUVELLE session** (nouveau conteneur). Carte source Canva de Fabrice = **canva.link/9rsjuebcvlkxhc3** (AG Starter PRO noir & or).
> **📣 POSTS GOOGLE BUSINESS (23/06) — fiche GBP VALIDÉE/VÉRIFIÉE** (catégorie « Concepteur de sites Web », zone Nantes, tél 07 44 82 95 16, 4 réseaux liés). **Hook RAPPEL LUNDI** ajouté (`.claude/settings.json`, `date +%u=1`) → chaque lundi, rappel de publier le post de la semaine. **Banque de 4 posts** rédigés (Devis gratuit / Conseil refonte / Sites dès 490 € / Réalisation), style noir & or « AG Starter PRO », QR → /contact, num 07 44 82 95 16. **À FAIRE quand le nouvel env tourne** : récupérer la carte Canva, l'utiliser comme base (ou Playfair via Chromium), sortir les 4 visuels. **SEO GBP à optimiser** (rappel #11) : avis 5★ RÉELS (jamais faux — outil `inc/ag-avis.php`/QR), photos vraies, catégories secondaires (agence marketing/SEO/pub), description + services + prix, posts hebdo. Schéma `ProfessionalService`/LocalBusiness Nantes DÉJÀ en place (`inc/ag-seo-meta.php`) — ne PAS ajouter d'`aggregateRating` tant qu'il n'y a pas de vrais avis affichés.
> **🛠️ FORMULAIRE CONTACT RÉPARÉ + CONVERSION GOOGLE ADS (23/06, déjà poussé)** : le formulaire `templates/page-contact.php` (action `ag_contact_form`) n'avait **AUCUN handler** → rien ne partait. Nouveau **`inc/ag-contact.php`** (chargé dans functions.php) : nonce+honeypot, email notif brandé, alerte SMS/Telegram (`ag_push`), .ics (`ag_calendar_notify`), ajout CRM (`ag_prospect_add_record` statut intéressé), redirection `/contact/?envoye=1`. **Conversion Google Ads** « Demande de devis » déclenchée sur `?envoye=1` via la balise de base (label `AW-18188842206/ZCMWCPfQvcEcEN7pjuFD`, option filtrable `ag_ads_conversion_devis`). Reste (rappel #12) : créer aussi conversions par appel (extension 07 44 82 95 16).
> **🔓 TEMPLATES — CONTENU DE DÉMO RENDU ÉDITABLE (23/06, déjà poussé, déblocage LOCKED.md autorisé par Fabrice)** : **Association** (events+combats accueil → 6 créneaux Customizer chacun, case Afficher ; bump 0.58.0) ; **Avocat** (compteurs + 3 citations Business éditables via Personnaliser, `pro-features.php` ; bump 1.1.18) ; **Restaurant** (stats/témoignages/FAQ override preset ; 1.24.5) ; **Coach** (idem ; 1.3.8) ; **Artisan** (idem + galerie réalisations 6 photos + photo équipe + timeline ; 1.15.7 ; ajouté à `.AG_FOCUS`). Resto+Coach : retiré l'affirmation codée en dur « Plus de 200 avis vérifiés » (sous-titre témoignages éditable, vide par défaut). **Chaque LOCKED.md annoté** du déblocage daté.
> **📦 COMPANION .org (23/06)** : review WordPress.org « AG Starter Companion » PENDED → corrigé `wporg-builds/ag-starter-companion/` : readme + en-tête traduits en ANGLAIS, `load_plugin_textdomain` supprimé, bump 1.13.0 + zip rebuild. Reste côté Fabrice : prouver propriété marque (répondre au relecteur depuis email @alliancegroupe-inc.com OU DNS). Thème avocat .org #277598 = statut `new` (file d'attente, rien à faire, juste attendre).
