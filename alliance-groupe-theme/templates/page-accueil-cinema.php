...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .av__rate{font-size:.95rem;color:#cdd4e2}
  .av__rate strong{font-family:var(--serif);font-size:1.6rem;color:var(--gold-hi);margin-right:8px;vertical-align:-2px}
  .av__stars{color:var(--gold);letter-spacing:.1em;margin-right:6px}
  ...
  .av__card:hover{border-color:rgba(212,180,92,.45)}
  ...
  .av__ava{width:42px;height:42px;flex:none;border-radius:50%;display:grid;place-items:center;overflow:hidden;
    background:linear-gradient(120deg,var(--gold),var(--gold-hi));color:#1a1206;font-weight:800;font-size:1.05rem}
  ...
  .av__q{color:#d7dae2;font-size:.93rem;line-height:1.65;margin:0}
...
<section class="rz" id="realisations">
  ...
  <div class="wrap rz__in">
    ...
    $ag_url   = function_exists( 'ag_geo_review_url' ) ? ag_geo_review_url() : (string) ( $ag_g['url'] ?? '' );
    $ag_avis  = array();
    foreach ( (array) ( $ag_g['reviews'] ?? array() ) as $ag_rv ) {
        $ag_txt = trim( (string) ( $ag_rv['text'] ?? '' ) );
        if ( '' === $ag_txt ) { continue; }
        $ag_nom = trim( (string) ( $ag_rv['author'] ?? '' ) );
        $ag_avis[] = array(
            'nom'   => $ag_nom ? $ag_nom : 'Client Google',
            'role'  => (string) ( $ag_rv['time'] ?? 'Avis Google' ),
            'photo' => (string) ( $ag_rv['photo'] ?? '' ),
            'txt'   => $ag_txt,
            'url'   => $ag_url,
            'lib'   => 'Voir sur Google →',
        );
    ...
    <div class="av" data-rv>
      ...
      <div class="av__grid">
        <?php foreach ( $ag_avis as $ag_a ) : ?>
        <article class="av__card">
          <div class="av__top">
            <span class="av__ava"><?php if ( ! empty( $ag_a['photo'] ) ) : ?><img src="<?php echo esc_url( $ag_a['photo'] ); ?>" alt="" loading="lazy" width="42" height="42"><?php else : ?><?php echo esc_html( mb_strtoupper( mb_substr( $ag_a['nom'], 0, 1 ) ) ); ?><?php endif; ?></span>
            <span class="av__who"><span class="av__nom"><?php echo esc_html( $ag_a['nom'] ); ?></span><span class="av__role"><?php echo esc_html( $ag_a['role'] ); ?></span></span>
          </div>
          <p class="av__q">«&nbsp;<?php echo esc_html( $ag_a['txt'] ); ?>&nbsp;»</p>
          <?php if ( ! empty( $ag_a['url'] ) ) : ?><a class="av__lien" href="<?php echo esc_url( $ag_a['url'] ); ?>" target="_blank" rel="noopener"><?php echo esc_html( $ag_a['lib'] ); ?></a><?php endif; ?>
        </article>
        <?php endforeach; ?>
      </div>
    </div>
