...
// Payment link URLs (still loaded for the companion plugin CTA + compatibility).
...
$ag_hub_metiers = array(
    ...
    array(
        ...
        'tagline'   => 'Manifeste, combats, calendrier événements, groupes locaux, dons, adhérents.',
...
/* Filtre extensible pour restreindre l'affichage à certains slugs.
   Par défaut : array() = tout afficher (les 6 métiers). Pour restreindre,
   hook ag_templates_only et retourne un tableau de slugs autorisés. */
$ag_templates_only = apply_filters( 'ag_templates_only', array() );
                    ...
                    <div style="display:flex;align-items:flex-start;gap:16px;margin-bottom:18px;">
                        <div style="flex:1;">
                            <h3 style="color:#fff;font-size:1.3rem;margin:0 0 4px;">
                                ...
                                <?php endif; ?>
                                <?php if ( ! empty( $m['is_carity'] ) ) : ?>
                                    <span style="display:inline-block;margin-left:6px;padding:2px 10px;background:#E10F1A;color:#fff;font-size:.7rem;font-weight:700;border-radius:100px;text-transform:uppercase;letter-spacing:.5px;vertical-align:middle;">Caritatif</span>
                                <?php endif; ?>
                            </h3>
