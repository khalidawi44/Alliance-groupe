<?php
/**
 * Auto-update du plugin AG Premium Domicile via raw GitHub.
 *
...
class AG_PD_Updater {
	const JSON_URL  = 'https://raw.githubusercontent.com/khalidawi44/Alliance-groupe/main/alliance-groupe-theme/assets/downloads/ag-premium-domicile.json';
	const SLUG      = 'ag-premium-domicile';
	const CACHE_KEY = 'ag_pd_remote_info';
	...
	public static function maybe_force_check() {
		if ( empty( $_GET['ag_pd_check_update'] ) || ! current_user_can( 'manage_options' ) ) return;
		delete_site_transient( 'update_plugins' );
		delete_transient( self::CACHE_KEY );
		wp_safe_redirect( admin_url( 'plugins.php?ag_pd_checked=1' ) );
		exit;
	}
	...
	public static function check_update( $transient ) {
		...
		$current = defined( 'AG_PREMIUM_DOMICILE_VERSION' ) ? AG_PREMIUM_DOMICILE_VERSION : '0.0.0';
	...
	public static function plugin_info( $result, $action, $args ) {
		...
		return (object) array(
			...
			'homepage'      => $remote['homepage']     ?? 'https://alliancegroupe-inc.com/wordpress-domicile',
			'sections'      => array(
				'description' => $remote['description'] ?? 'AG Premium Domicile',
