<?php
/**
 * Auto-update du plugin AG Premium Avocat via raw GitHub.
 */
if ( ! defined( 'ABSPATH' ) ) exit;

class AG_PA_Updater {
	const JSON_URL  = 'https://raw.githubusercontent.com/khalidawi44/Alliance-groupe/main/alliance-groupe-theme/assets/downloads/ag-premium-avocat.json';
	const SLUG      = 'ag-premium-avocat';
	const CACHE_KEY = 'ag_pa_remote_info';
	...
	public static function maybe_force_check() {
		if ( empty( $_GET['ag_pa_check_update'] ) || ! current_user_can( 'manage_options' ) ) return;
		delete_site_transient( 'update_plugins' );
		delete_transient( self::CACHE_KEY );
		wp_safe_redirect( admin_url( 'plugins.php?ag_pa_checked=1' ) );
		exit;
	}
	...
	public static function check_update( $transient ) {
		...
		$current = defined( 'AG_PREMIUM_AVOCAT_VERSION' ) ? AG_PREMIUM_AVOCAT_VERSION : '0.0.0';
	...
	public static function plugin_info( $result, $action, $args ) {
		...
		return (object) array(
			...
			'sections'      => array(
				'description' => $remote['description'] ?? 'AG Premium Avocat',
