...
- 2026-09-21 · CODE · **⚠️ À TRANCHER LANE DESIGN — le « waouh » de /refais-mon-site est bridé.** L'analyse du tunnel a confirmé (code lu) : la page promet « te montre TOUT DE SUITE… sans inscription » (`ag-refais-mon-site.php:224`) mais la vraie maquette est **floutée** (`agrg-flou`, blur 7px) et exige email + clic de confirmation dans un mail avant de la voir en clair (`ag-refais-acces.php:94` retire le HTML du payload, `:143` « un seul clic pour la voir sans flou »). Un double opt-in perd 60-80 % entre email saisi et email confirmé. **Décision visuelle/produit = votre lane :** soit on montre la maquette immédiatement (waouh plein, capture après), soit on assume le flou (moins de leads mais emails confirmés). CÔTÉ CODE (déjà fait, ma lane) : le lead est désormais capté dès la 1re interaction via `ag_prospect_upsert` + relance chaude, donc même un non-cliqueur n'est plus perdu. Reste le choix d'AFFICHAGE, qui est à vous.

- 2026-09-22 · CODE → DESIGN · **Livret « Formation Ambassadeur » LUXE : corriger le chiffre de commission.** Le PDF hébergé à `wp-content/uploads/2026/09/Formation-Ambassadeur-50.pdf` (design noir & or, lion AG, art-déco — votre version Cowork) affiche « **20 %** » (couverture « 20 % DE COMMISSION SUR CHAQUE VENTE » + « ÉDITION 20 % » + titre onglet « 20 % »). Il faut la version **50 %**. → Depuis VOTRE source, changer le nombre 20 → 50 (couverture + « ÉDITION » + toute autre occurrence) et **ré-exporter en gardant le même nom de fichier** `Formation-Ambassadeur-50.pdf`, puis ré-uploader dans Médias (écraser) pour conserver l'URL. Je ne peux pas éditer le chiffre côté CODE : le PDF final (2,9 Mo) a des polices sous-ensemblées + images, le texte n'est pas modifiable proprement sans la source. Idéalement produire les 3 éditions cohérentes (10/20/50) au même gabarit. CÔTÉ CODE (fait) : l'envoi de la formation choisit désormais automatiquement le PDF **du taux** de chaque ambassadeur (3 champs de liens 10/20/50 dans Ambassadeurs → 📚 Formation) — donc dès que le bon fichier 50 % est en ligne, tout est cohérent.
