...
/**
 * Helper: read a Customizer option (short alias for ag_starter_restaurant_get_option).
 */
if ( ! function_exists( 'ag_restaurant_opt' ) ) {
	function ag_restaurant_opt( $key, $default = '' ) {
 ...
 */
if ( ! function_exists( 'ag_restaurant_page_section_text' ) ) {
	function ag_restaurant_page_section_text( $slug, $fallback_h2, $fallback_lead = '' ) {
 ...
 */
if ( ! function_exists( 'ag_restaurant_render_split_title' ) ) {
	function ag_restaurant_render_split_title( $title ) {
...
function ag_starter_restaurant_companion_notice() {
	...
	<div class="ag-welcome-banner" style="background:linear-gradient(135deg,#1a1a2e 0%,#0a0a0f 100%);border:1px solid rgba(212,180,92,.3);border-left:4px solid #D4B45C;border-radius:8px;padding:40px 36px;margin:20px 20px 20px 0;display:flex;align-items:center;gap:32px;flex-wrap:wrap;">
		<div style="flex:1;min-width:280px;">
			<h2 style="color:#fff;font-size:1.6rem;margin:0 0 12px;">🎉 <?php esc_html_e( 'Bienvenue dans AG Starter Restaurant !', 'ag-starter-restaurant' ); ?></h2>
			<p style="color:rgba(255,255,255,.7);font-size:1.05rem;line-height:1.6;margin:0 0 8px;"><?php esc_html_e( 'Votre theme est installe. Pour un site pret a l\'emploi en 1 clic (pages, menu, reglages), installez le plugin gratuit AG Starter Companion.', 'ag-starter-restaurant' ); ?></p>
			<ul style="color:rgba(255,255,255,.6);font-size:.92rem;margin:12px 0 0;padding-left:18px;">
				<li><?php esc_html_e( 'Pages creees automatiquement (Accueil, Prestations, Realisations, A propos, Contact)', 'ag-starter-restaurant' ); ?></li>
		...
		<div style="text-align:center;">
			<a href="<?php echo esc_url( $search_url ); ?>" style="display:inline-block;background:#D4B45C;color:#0a0a0f;font-size:1.05rem;font-weight:700;padding:16px 32px;border-radius:8px;text-decoration:none;box-shadow:0 4px 16px rgba(212,180,92,.3);"><?php esc_html_e( 'Télécharger AG Starter Companion', 'ag-starter-restaurant' ); ?></a>
			<p style="color:rgba(255,255,255,.4);font-size:.8rem;margin-top:10px;"><?php esc_html_e( 'Téléchargez le ZIP, puis Extensions → Ajouter → Téléverser', 'ag-starter-restaurant' ); ?></p>
		</div>
	</div>
	<?php
}
...
function ag_starter_restaurant_dashboard_widget() {
	...
	wp_add_dashboard_widget( 'ag_starter_welcome', esc_html__( '🚀 AG Starter Restaurant — Configuration', 'ag-starter-restaurant' ), 'ag_starter_restaurant_dashboard_widget_render' );
...
if ( ! function_exists( 'ag_restaurant_credit' ) ) :
	function ag_restaurant_credit() {
		...
		printf(
			wp_kses( __( 'Création du site : %s', 'ag-starter-restaurant' ), array( 'a' => array( 'href' => array(), 'title' => array() ) ) ),
			'<a href="https://alliancegroupe-inc.com/wordpress-restaurant" title="Création de site WordPress pour restaurant par Alliance Groupe">Alliance Groupe</a>'
		);
		echo ' &middot; <a href="https://alliancegroupe-inc.com" title="Agence web et IA Alliance Groupe">' . esc_html__( 'Agence Web & IA', 'ag-starter-restaurant' ) . '</a>';
		echo '</small></div>';
	}
endif;
