 ...
 * barber/domicile/resto) que l'utilisateur peut avoir laisse en switchant entre
 ...
 *
 * Apres reset : seules les options ag_domicile_*, les pages artisan
...
class ag_domicile_Reset {

	/**
	 * Prefixes theme_mod d'autres templates Alliance Groupe — supprimes
	 * lors du reset (laisse ag_domicile_*, background_color, blogname, etc.).
	 */
	...
	const FOREIGN_PAGE_SLUGS = array(
		...
		'tarifs-domicile', 'seances',
		// Pages du template domicile BASIQUE d'origine — à remplacer par les pages premium
	...
	const ARTISAN_PAGES = array(
		'prestations'        => array( 'title' => 'Nos prestations',    'content' => "Aide au lever et au coucher, aide à la toilette, préparation des repas, entretien du logement, courses, compagnie, garde de nuit et accompagnement aux sorties. Des prestations adaptées à chaque situation, éligibles au crédit d'impôt de 50%." ),
		'zones-intervention' => array( 'title' => "Zones d'intervention", 'content' => "Nous intervenons à Nantes et dans toute son agglomération, 7j/7, en journée comme la nuit. Délais de mise en place courts, y compris en sortie d'hospitalisation. Votre commune n'apparaît pas ? Contactez-nous." ),
		'realisations'       => array( 'title' => 'Témoignages',         'content' => "Découvrez ce qu'en disent les familles que nous accompagnons : un proche qui reste chez lui, en sécurité et entouré. Maintien à domicile, répit pour les aidants, lutte contre l'isolement — chaque histoire est unique." ),
		'qui-sommes-nous'    => array( 'title' => 'À propos',            'content' => "Service déclaré d'aide et d'accompagnement à domicile, nous permettons à chacun de bien vieillir chez soi. Intervenants qualifiés et suivis, référent stable, plan d'aide sur-mesure. Évaluation à domicile gratuite et sans engagement." ),
		'devis'              => array( 'title' => 'Demander un devis',  'content' => '[ag_domicile_devis]' ),
		'mentions'           => array( 'title' => 'Mentions légales',   'content' => 'Mentions légales et informations sur la société.' ),
		'contact'            => array( 'title' => 'Contact',            'content' => "Pour une évaluation gratuite à domicile ou toute question, contactez-nous. Réponse rapide, sans engagement." ),
	);
	 ...
	 * ag_domicile_installer_version.
	 */
	const INSTALLER_VERSION = 3;

	/**
	 * Anciens titres de pages (artisan ou template basique) qui doivent être
	 * migrés vers les nouveaux titres domicile lors de install_pages_and_menu.
	 */
	...
	public static function init() {
		...
		add_action( 'admin_post_ag_domicile_reset', array( __CLASS__, 'handle_reset' ) );
		add_action( 'admin_post_ag_domicile_sync_reset', array( __CLASS__, 'handle_sync_reset' ) );
	...
	public static function auto_install_pages() {
		...
		self::auto_apply_gwen_preset();
		update_option( 'ag_domicile_installer_version', self::INSTALLER_VERSION );
	}

	/**
	 * Applique automatiquement la config Gwen (preset par defaut) a la premiere
	 * activation, pour un site pret a l'emploi sans manipulation. N'ecrase rien
	 * si un metier a deja ete choisi.
	 */
	public static function auto_apply_gwen_preset() {
		if ( get_theme_mod( 'ag_domicile_metier_slug', '' ) ) {
			return;
		}
		if ( ! class_exists( 'AG_Domicile_Presets' ) ) {
			return;
		}
		$presets = AG_Domicile_Presets::get_presets();
		if ( empty( $presets['domicile_familles'] ) ) {
			return;
		}
		$p = $presets['domicile_familles'];
		foreach ( $p['mods'] as $k => $v ) {
			if ( is_string( $v ) && 0 === strpos( $v, 'THEME:' ) ) {
				$v = get_template_directory_uri() . substr( $v, 6 );
			}
			set_theme_mod( $k, $v );
		}
		set_theme_mod( 'ag_domicile_metier_slug', 'domicile_familles' );
		set_theme_mod( 'ag_domicile_services_json', wp_json_encode( $p['services'] ) );
		if ( ! empty( $p['pages'] ) && is_array( $p['pages'] ) ) {
			foreach ( $p['pages'] as $slug => $content ) {
				...
				wp_update_post( array( 'ID' => $page->ID, 'post_content' => trim( $wrapped ) ) );
			}
		}
	}
	...
	public static function on_upgrade( $upgrader, $hook_extra ) {
		...
		if ( ! in_array( 'ag-gwen-services', (array) $hook_extra['themes'], true ) ) return;
		// Reset le flag pour forcer maybe_install à relancer au prochain init.
		delete_option( 'ag_domicile_installer_version' );
	}
	...
	public static function maybe_install() {
		...
		$stored = (int) get_option( 'ag_domicile_installer_version', 0 );
	...
	/**
	 * Crée les pages domicile manquantes ET reconstruit le menu primary.
	...
	public static function install_pages_and_menu() {
		...
		foreach ( self::ARTISAN_PAGES as $slug => $data ) {
			...
			if ( ! $page ) {
				...
				if ( '[ag_domicile_devis]' === trim( $data['content'] ) ) {
					$current = get_post_field( 'post_content', $page->ID );
					if ( false === strpos( $current, 'wp:shortcode' ) && false === strpos( $current, '[ag_domicile_devis]' ) ) {
		...
		$menu_name = 'AG Domicile — Principal';
	...
	public static function register_menu() {
		add_theme_page(
			__( 'Réinitialiser le thème', 'ag-gwen-services' ),
			'🔄 ' . __( 'Réinitialiser', 'ag-gwen-services' ),
			'manage_options',
			'ag-domicile-reset',
	...
	public static function render() {
		...
		$stats     = ( $done || $sync_done ) ? get_transient( 'ag_domicile_reset_stats' ) : null;
		?>
		<div class="wrap">
			<h1>🔄 <?php esc_html_e( 'Réinitialiser le thème — Gwen Services', 'ag-gwen-services' ); ?></h1>
			...
			<div style="background:linear-gradient(135deg,#0a0a0f 0%,#1a1a2e 100%);border-radius:12px;padding:28px 32px;margin:20px 0;color:#fff;max-width:820px;">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" onsubmit="return confirm('Lancer le combo SYNC GITHUB + RESET ? Tous les caches seront purges et les residus supprimes.');">
					<input type="hidden" name="action" value="ag_domicile_sync_reset" />
					<?php wp_nonce_field( 'ag_domicile_sync_reset' ); ?>
					<button type="submit" class="button button-primary button-hero" style="background:#F37A1F;border-color:#F37A1F;color:#fff;font-weight:700;padding:0 30px;">
						🚀 <?php esc_html_e( 'SYNC GITHUB + RESET COMPLET', 'ag-gwen-services' ); ?>
					</button>
				</form>
			</div>
			<hr style="margin:30px 0;border:none;border-top:1px solid #ddd;">
			<h2><?php esc_html_e( 'Ou : reset seul (sans sync GitHub)', 'ag-gwen-services' ); ?></h2>

			<?php if ( ( $done || $sync_done ) && $stats ) : ?>
				<div class="notice notice-success">
					<p><strong>✅ <?php echo $sync_done ? esc_html__( 'Sync GitHub + reset complet effectués.', 'ag-gwen-services' ) : esc_html__( 'Réinitialisation effectuée.', 'ag-gwen-services' ); ?></strong></p>
					<?php if ( $sync_done ) : ?>
						<p>🔄 <?php esc_html_e( 'Caches mises à jour purgés (theme + companion + LiteSpeed). Si une nouvelle version est disponible, elle apparaîtra dans', 'ag-gwen-services' ); ?> <a href="<?php echo esc_url( admin_url( 'themes.php' ) ); ?>"><?php esc_html_e( 'Apparence > Thèmes', 'ag-gwen-services' ); ?></a>.</p>
					<?php endif; ?>
					<ul style="margin-left:20px;list-style:disc;">
						<li><?php printf( esc_html__( '%d options Customizer d\'autres templates supprimées', 'ag-gwen-services' ), (int) $stats['mods_deleted'] ); ?></li>
						<li><?php printf( esc_html__( '%d entrées CPT (combats/événements/pétitions/etc.) supprimées', 'ag-gwen-services' ), (int) $stats['cpt_deleted'] ); ?></li>
						<li><?php printf( esc_html__( '%d pages d\'autres templates supprimées', 'ag-gwen-services' ), (int) $stats['pages_deleted'] ); ?></li>
						<li><?php printf( esc_html__( '%d pages artisan créées/restaurées', 'ag-gwen-services' ), (int) $stats['pages_created'] ); ?></li>
						<li><?php esc_html_e( 'Menu principal reconstruit (6 items artisan)', 'ag-gwen-services' ); ?></li>
					</ul>
				</div>
			<?php endif; ?>
			...
			<div style="background:#fff;border:1px solid #ccd0d4;border-left:4px solid #d63638;padding:20px 24px;margin:20px 0;max-width:820px;">
				<h2 style="margin-top:0;color:#d63638;">⚠️ <?php esc_html_e( 'Action destructive', 'ag-gwen-services' ); ?></h2>
				<p><?php esc_html_e( 'Ce bouton supprime tous les résidus laissés par d\'autres templates Alliance Groupe (Association, Avocat, Barber, Domicile, Restaurant) que vous avez pu activer précédemment :', 'ag-gwen-services' ); ?></p>
				<ul style="margin-left:24px;list-style:disc;">
					<li><?php esc_html_e( 'Options Customizer aux préfixes ag_asso_*, ag_avocat_*, ag_barber_*, ag_domicile_*, ag_resto_*, ag_fid_*', 'ag-gwen-services' ); ?></li>
					<li><?php esc_html_e( 'Tous les CPT : combats, événements, groupes, pétitions, PV, domaines avocat', 'ag-gwen-services' ); ?></li>
					<li><?php esc_html_e( 'Pages WP : Manifeste, Combats, Événements, Pétitions, Don, Adhérer, Domaines, Honoraires, Carte, Réservation, etc.', 'ag-gwen-services' ); ?></li>
					<li><?php esc_html_e( 'Menu principal : reconstruit avec uniquement les items artisan', 'ag-gwen-services' ); ?></li>
				</ul>
				<p><strong><?php esc_html_e( 'Ce qui est conservé :', 'ag-gwen-services' ); ?></strong> <?php esc_html_e( 'vos options ag_domicile_*, vos pages artisan (Prestations, Zones, Réalisations, Qui sommes-nous, Mentions, Contact), vos articles de blog, vos utilisateurs, vos plugins.', 'ag-gwen-services' ); ?></p>
				<p><?php esc_html_e( 'Si une page artisan manquait, elle est recréée avec un contenu placeholder.', 'ag-gwen-services' ); ?></p>
			</div>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" onsubmit="return confirm('Confirmer la réinitialisation ? Tous les contenus d\'autres templates seront supprimés définitivement.');">
				<input type="hidden" name="action" value="ag_domicile_reset" />
				<?php wp_nonce_field( 'ag_domicile_reset' ); ?>
				<p>
					<label>
						<input type="checkbox" name="ag_domicile_reset_confirm" value="1" required />
						<?php esc_html_e( 'J\'ai compris — supprimer définitivement tous les résidus d\'autres templates.', 'ag-gwen-services' ); ?>
					</label>
				</p>
				<p>
					<button type="submit" class="button button-primary button-hero" style="background:#d63638;border-color:#d63638;">
						🔄 <?php esc_html_e( 'TOUT RESET — remettre le thème à l\'état d\'origine', 'ag-gwen-services' ); ?>
					</button>
				</p>
			</form>
		</div>
	...
	public static function handle_sync_reset() {
		...
		check_admin_referer( 'ag_domicile_sync_reset' );

		// 1. Force-check GitHub theme (transients de l'updater + WP)
		delete_site_transient( 'update_themes' );
		delete_site_transient( 'update_plugins' );
		// AG_Theme_Updater::CACHE_KEY = 'ag_domicile_theme_remote' (cf
		// inc/theme-updater.php). On purge ce nom exact.
		delete_transient( 'ag_domicile_theme_remote' );
		...
		set_transient( 'ag_domicile_reset_stats', $stats, 60 );
		wp_safe_redirect( admin_url( 'themes.php?page=ag-domicile-reset&sync_done=1' ) );
		exit;
	}
	...
	public static function handle_reset() {
		if ( ! current_user_can( 'manage_options' ) ) wp_die( 'forbidden' );
		check_admin_referer( 'ag_domicile_reset' );
		if ( empty( $_POST['ag_domicile_reset_confirm'] ) ) {
			wp_safe_redirect( admin_url( 'themes.php?page=ag-domicile-reset' ) );
			exit;
		}
		...
		if ( ! is_wp_error( $menu_id ) ) {
			...
			wp_update_nav_menu_item( $menu_id, 0, array(
				'menu-item-title'  => __( 'Accueil', 'ag-gwen-services' ),
				'menu-item-url'    => home_url( '/' ),
				'menu-item-type'   => 'custom',
				'menu-item-status' => 'publish',
			) );
		...
		set_transient( 'ag_domicile_reset_stats', $stats, 60 );
		wp_safe_redirect( admin_url( 'themes.php?page=ag-domicile-reset&reset_done=1' ) );
		exit;
	}
}
ag_domicile_Reset::init();
