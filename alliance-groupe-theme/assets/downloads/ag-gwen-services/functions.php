<?php
/**
 * Gwen Services functions and definitions.
 *
...
if ( ! function_exists( 'ag_starter_domicile_setup' ) ) :
	...
	function ag_starter_domicile_setup() {
		 ...
		 */
		load_theme_textdomain( 'ag-gwen-services', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Menu principal', 'ag-gwen-services' ),
			)
		);
	}
endif;
...
function ag_starter_domicile_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Barre laterale', 'ag-gwen-services' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Ajoutez vos widgets ici.', 'ag-gwen-services' ),
...
add_action( 'after_setup_theme', function () {
    ...
    new AG_Theme_Updater( 'ag-gwen-services', wp_get_theme()->get( 'Version' ) );
    $GLOBALS['ag_pro'] = new AG_Pro_Features( 'ag-gwen-services' );
}, 20 );
...
function ag_starter_domicile_companion_notice() {
	...
	<div class="ag-welcome-banner" style="background:linear-gradient(135deg,#1a1a2e 0%,#0a0a0f 100%);border:1px solid rgba(212,180,92,.3);border-left:4px solid #D4B45C;border-radius:8px;padding:40px 36px;margin:20px 20px 20px 0;display:flex;align-items:center;gap:32px;flex-wrap:wrap;">
		<div style="flex:1;min-width:280px;">
			<h2 style="color:#fff;font-size:1.6rem;margin:0 0 12px;">🎉 <?php esc_html_e( 'Bienvenue dans Gwen Services !', 'ag-gwen-services' ); ?></h2>
			<p style="color:rgba(255,255,255,.7);font-size:1.05rem;line-height:1.6;margin:0 0 8px;"><?php esc_html_e( 'Votre theme est installe. Pour un site pret a l\'emploi en 1 clic (pages, menu, reglages), installez le plugin gratuit AG Starter Companion.', 'ag-gwen-services' ); ?></p>
			<ul style="color:rgba(255,255,255,.6);font-size:.92rem;margin:12px 0 0;padding-left:18px;">
				<li><?php esc_html_e( 'Pages creees automatiquement (Accueil, Prestations, Zones d\'intervention, Temoignages, Devis, Contact)', 'ag-gwen-services' ); ?></li>
				<li><?php esc_html_e( 'Menu principal configure et assigne', 'ag-gwen-services' ); ?></li>
				<li><?php esc_html_e( 'Page d\'accueil et permaliens actives', 'ag-gwen-services' ); ?></li>
				<li><?php esc_html_e( '100% gratuit, aucune connexion internet requise', 'ag-gwen-services' ); ?></li>
			</ul>
		</div>
		<div style="text-align:center;">
			<a href="<?php echo esc_url( $search_url ); ?>" style="display:inline-block;background:#D4B45C;color:#0a0a0f;font-size:1.05rem;font-weight:700;padding:16px 32px;border-radius:8px;text-decoration:none;box-shadow:0 4px 16px rgba(212,180,92,.3);"><?php esc_html_e( 'Télécharger AG Starter Companion', 'ag-gwen-services' ); ?></a>
			<p style="color:rgba(255,255,255,.4);font-size:.8rem;margin-top:10px;"><?php esc_html_e( 'Téléchargez le ZIP, puis Extensions → Ajouter → Téléverser', 'ag-gwen-services' ); ?></p>
		</div>
	</div>
	<?php
}
...
function ag_starter_domicile_dashboard_widget() {
	...
	wp_add_dashboard_widget( 'ag_starter_welcome', esc_html__( '🚀 Gwen Services — Configuration', 'ag-gwen-services' ), 'ag_starter_domicile_dashboard_widget_render' );
...
function ag_starter_domicile_dashboard_widget_render() {
	...
	<div style="text-align:center;padding:20px 0;">
		<p style="font-size:1.15rem;margin:0 0 16px;"><strong><?php esc_html_e( 'Votre theme est pret !', 'ag-gwen-services' ); ?></strong></p>
		<p style="color:#666;margin:0 0 20px;"><?php esc_html_e( 'Installez le plugin gratuit AG Starter Companion pour creer automatiquement vos pages, votre menu et configurer votre site en 1 clic.', 'ag-gwen-services' ); ?></p>
		<a href="<?php echo esc_url( $search_url ); ?>" class="button button-primary button-hero"><?php esc_html_e( 'Télécharger AG Starter Companion', 'ag-gwen-services' ); ?></a>
		<p style="color:#999;font-size:.85rem;margin-top:12px;"><?php esc_html_e( 'Gratuit — 10 secondes — aucune inscription', 'ag-gwen-services' ); ?></p>
	</div>
	<?php
}
...
if ( ! function_exists( 'ag_domicile_credit' ) ) :
	function ag_domicile_credit() {
		...
		printf(
			wp_kses( __( 'Site WordPress conçu par %s', 'ag-gwen-services' ), array( 'a' => array( 'href' => array(), 'title' => array() ) ) ),
		...
		echo ' &middot; <a href="https://alliancegroupe-inc.com/bureau-nantes" title="Agence web Alliance Groupe à Nantes">' . esc_html__( 'Agence Web Nantes', 'ag-gwen-services' ) . '</a>';
		echo '</small></div>';
	}
endif;
