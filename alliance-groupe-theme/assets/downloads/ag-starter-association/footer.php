...
(function () {
    ...
    var menus = document.querySelectorAll( '.ag-asso-header__menu li, .ag-asso-footer__menu li' );
    Array.prototype.forEach.call( menus, function ( li ) {
        var a = li.querySelector( 'a' );
        if ( ! a ) return;
        var href = a.getAttribute( 'href' );
        if ( ! href ) return;
        // Compare le path uniquement (sans domaine)
        var aPath = '';
        try {
            aPath = new URL( a.href ).pathname.replace(/\/+$/, '') || '/';
        } catch ( e ) {
            aPath = href.replace(/\/+$/, '') || '/';
        }
