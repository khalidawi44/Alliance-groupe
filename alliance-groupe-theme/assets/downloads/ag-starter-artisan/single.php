		...
		while ( have_posts() ) :
			...
			<article <?php post_class(); ?>>
				...
				<div class="ag-entry-content">
					...
					wp_link_pages(
						array(
							'before' => '<div class="ag-page-links">' . esc_html__( 'Pages :', 'ag-starter-artisan' ),
							'after'  => '</div>',
						)
					);
					?>
				...
				<footer class="ag-entry-footer">
					...
					if ( $categories ) {
						printf( '<p>%s %s</p>', esc_html__( 'Categories :', 'ag-starter-artisan' ), wp_kses_post( $categories ) );
					}
					$tags = get_the_tag_list( '', ', ' );
					if ( $tags ) {
						printf( '<p>%s %s</p>', esc_html__( 'Etiquettes :', 'ag-starter-artisan' ), wp_kses_post( $tags ) );
					}
					?>
				</footer>
			</article>
