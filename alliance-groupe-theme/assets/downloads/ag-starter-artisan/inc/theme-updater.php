<?php
/**
 * Auto-update du theme AG Starter Artisan via raw GitHub.
 */
if ( ! defined( 'ABSPATH' ) ) exit;

class AG_Artisan_Theme_Updater {
	const JSON_URL  = 'https://raw.githubusercontent.com/khalidawi44/Alliance-groupe/main/alliance-groupe-theme/assets/downloads/ag-starter-artisan.json';
	const SLUG      = 'ag-starter-artisan';
	const CACHE_KEY = 'ag_artisan_theme_remote';
	...
	public static function maybe_force_check() {
		if ( empty( $_GET['ag_artisan_check_theme'] ) || ! current_user_can( 'manage_options' ) ) return;
		delete_site_transient( 'update_themes' );
		delete_transient( self::CACHE_KEY );
		wp_safe_redirect( admin_url( 'themes.php?ag_artisan_theme_checked=1' ) );
		exit;
	}
	...
	public static function check_update( $transient ) {
		...
		if ( version_compare( $remote['version'], $current, '>' ) ) {
			$transient->response[ self::SLUG ] = array(
				...
				'url'         => $remote['homepage'] ?? 'https://alliancegroupe-inc.com/wordpress-artisan',
