...
class ag_domicile_Reset {
	...
	public static function on_upgrade( $upgrader, $hook_extra ) {
		...
		if ( ! in_array( 'ag-starter-domicile', (array) $hook_extra['themes'], true ) ) return;
	...
	public static function register_menu() {
		add_theme_page(
			__( 'Réinitialiser le thème', 'ag-starter-domicile' ),
			'🔄 ' . __( 'Réinitialiser', 'ag-starter-domicile' ),
			'manage_options',
			'ag-domicile-reset',
	...
	public static function render() {
		...
		?>
		<div class="wrap">
			<h1>🔄 <?php esc_html_e( 'Réinitialiser le thème — AG Starter Domicile', 'ag-starter-domicile' ); ?></h1>
			...
			<div style="background:linear-gradient(135deg,#0a0a0f 0%,#1a1a2e 100%);border-radius:12px;padding:28px 32px;margin:20px 0;color:#fff;max-width:820px;">
				...
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" onsubmit="return confirm('Lancer le combo SYNC GITHUB + RESET ? Tous les caches seront purges et les residus supprimes.');">
					...
					<button type="submit" class="button button-primary button-hero" style="background:#F37A1F;border-color:#F37A1F;color:#fff;font-weight:700;padding:0 30px;">
						🚀 <?php esc_html_e( 'SYNC GITHUB + RESET COMPLET', 'ag-starter-domicile' ); ?>
					</button>
				</form>
			</div>
			<hr style="margin:30px 0;border:none;border-top:1px solid #ddd;">
			<h2><?php esc_html_e( 'Ou : reset seul (sans sync GitHub)', 'ag-starter-domicile' ); ?></h2>

			<?php if ( ( $done || $sync_done ) && $stats ) : ?>
				<div class="notice notice-success">
					<p><strong>✅ <?php echo $sync_done ? esc_html__( 'Sync GitHub + reset complet effectués.', 'ag-starter-domicile' ) : esc_html__( 'Réinitialisation effectuée.', 'ag-starter-domicile' ); ?></strong></p>
					<?php if ( $sync_done ) : ?>
						<p>🔄 <?php esc_html_e( 'Caches mises à jour purgés (theme + companion + LiteSpeed). Si une nouvelle version est disponible, elle apparaîtra dans', 'ag-starter-domicile' ); ?> <a href="<?php echo esc_url( admin_url( 'themes.php' ) ); ?>"><?php esc_html_e( 'Apparence > Thèmes', 'ag-starter-domicile' ); ?></a>.</p>
					<?php endif; ?>
					<ul style="margin-left:20px;list-style:disc;">
						<li><?php printf( esc_html__( '%d options Customizer d\'autres templates supprimées', 'ag-starter-domicile' ), (int) $stats['mods_deleted'] ); ?></li>
						<li><?php printf( esc_html__( '%d entrées CPT (combats/événements/pétitions/etc.) supprimées', 'ag-starter-domicile' ), (int) $stats['cpt_deleted'] ); ?></li>
						<li><?php printf( esc_html__( '%d pages d\'autres templates supprimées', 'ag-starter-domicile' ), (int) $stats['pages_deleted'] ); ?></li>
						<li><?php printf( esc_html__( '%d pages artisan créées/restaurées', 'ag-starter-domicile' ), (int) $stats['pages_created'] ); ?></li>
						<li><?php esc_html_e( 'Menu principal reconstruit (6 items artisan)', 'ag-starter-domicile' ); ?></li>
					</ul>
				</div>
			<?php endif; ?>
			...
			<div style="background:#fff;border:1px solid #ccd0d4;border-left:4px solid #d63638;padding:20px 24px;margin:20px 0;max-width:820px;">
				<h2 style="margin-top:0;color:#d63638;">⚠️ <?php esc_html_e( 'Action destructive', 'ag-starter-domicile' ); ?></h2>
				<p><?php esc_html_e( 'Ce bouton supprime tous les résidus laissés par d\'autres templates Alliance Groupe (Association, Avocat, Barber, Domicile, Restaurant) que vous avez pu activer précédemment :', 'ag-starter-domicile' ); ?></p>
				<ul style="margin-left:24px;list-style:disc;">
					<li><?php esc_html_e( 'Options Customizer aux préfixes ag_asso_*, ag_avocat_*, ag_barber_*, ag_domicile_*, ag_resto_*, ag_fid_*', 'ag-starter-domicile' ); ?></li>
					<li><?php esc_html_e( 'Tous les CPT : combats, événements, groupes, pétitions, PV, domaines avocat', 'ag-starter-domicile' ); ?></li>
					<li><?php esc_html_e( 'Pages WP : Manifeste, Combats, Événements, Pétitions, Don, Adhérer, Domaines, Honoraires, Carte, Réservation, etc.', 'ag-starter-domicile' ); ?></li>
					<li><?php esc_html_e( 'Menu principal : reconstruit avec uniquement les items artisan', 'ag-starter-domicile' ); ?></li>
				</ul>
				<p><strong><?php esc_html_e( 'Ce qui est conservé :', 'ag-starter-domicile' ); ?></strong> <?php esc_html_e( 'vos options ag_domicile_*, vos pages artisan (Prestations, Zones, Réalisations, Qui sommes-nous, Mentions, Contact), vos articles de blog, vos utilisateurs, vos plugins.', 'ag-starter-domicile' ); ?></p>
				<p><?php esc_html_e( 'Si une page artisan manquait, elle est recréée avec un contenu placeholder.', 'ag-starter-domicile' ); ?></p>
			</div>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" onsubmit="return confirm('Confirmer la réinitialisation ? Tous les contenus d\'autres templates seront supprimés définitivement.');">
				...
				<p>
					<label>
						...
						<?php esc_html_e( 'J\'ai compris — supprimer définitivement tous les résidus d\'autres templates.', 'ag-starter-domicile' ); ?>
					</label>
				</p>
				<p>
					<button type="submit" class="button button-primary button-hero" style="background:#d63638;border-color:#d63638;">
						🔄 <?php esc_html_e( 'TOUT RESET — remettre le thème à l\'état d\'origine', 'ag-starter-domicile' ); ?>
					</button>
				</p>
			</form>
		</div>
	...
	public static function handle_reset() {
		...
		if ( ! is_wp_error( $menu_id ) ) {
			...
			wp_update_nav_menu_item( $menu_id, 0, array(
				'menu-item-title'  => __( 'Accueil', 'ag-starter-domicile' ),
				'menu-item-url'    => home_url( '/' ),
				'menu-item-type'   => 'custom',
				'menu-item-status' => 'publish',
			) );
