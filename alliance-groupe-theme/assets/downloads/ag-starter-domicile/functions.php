<?php
/**
 * AG Starter Domicile functions and definitions.
 *
...
if ( ! function_exists( 'ag_starter_domicile_setup' ) ) :
	/**
	 * Sets up theme defaults and registers support for various WordPress features.
	 */
	function ag_starter_domicile_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 */
		load_theme_textdomain( 'ag-starter-domicile', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Menu principal', 'ag-starter-domicile' ),
			)
		);
	}
endif;
add_action( 'after_setup_theme', 'ag_starter_domicile_setup' );

/**
 * Register widget area.
 */
function ag_starter_domicile_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Barre laterale', 'ag-starter-domicile' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Ajoutez vos widgets ici.', 'ag-starter-domicile' ),
...
add_action( 'widgets_init', 'ag_starter_domicile_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function ag_starter_domicile_scripts() {
	wp_enqueue_style(
		'ag-starter-domicile-style',
		get_stylesheet_uri(),
		array(),
...
add_action( 'wp_enqueue_scripts', 'ag_starter_domicile_scripts' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function ag_starter_domicile_pingback_header() {
...
if ( ! function_exists( 'ag_domicile_opt' ) ) {
	function ag_domicile_opt( $key, $default = '' ) {
 ...
 */
if ( ! function_exists( 'ag_domicile_page_section_text' ) ) {
	function ag_domicile_page_section_text( $slug, $fallback_h2, $fallback_lead = '' ) {
 ...
 */
if ( ! function_exists( 'ag_domicile_render_split_title' ) ) {
	function ag_domicile_render_split_title( $title ) {
...
add_action( 'after_setup_theme', function () {
    ...
    new AG_Theme_Updater( 'ag-starter-domicile', wp_get_theme()->get( 'Version' ) );
    $GLOBALS['ag_pro'] = new AG_Pro_Features( 'ag-starter-domicile' );
}, 20 );
 ...
 */
function ag_starter_domicile_companion_notice() {
	...
	<div class="ag-welcome-banner" style="background:linear-gradient(135deg,#1a1a2e 0%,#0a0a0f 100%);border:1px solid rgba(212,180,92,.3);border-left:4px solid #D4B45C;border-radius:8px;padding:40px 36px;margin:20px 20px 20px 0;display:flex;align-items:center;gap:32px;flex-wrap:wrap;">
		<div style="flex:1;min-width:280px;">
			<h2 style="color:#fff;font-size:1.6rem;margin:0 0 12px;">🎉 <?php esc_html_e( 'Bienvenue dans AG Starter Domicile !', 'ag-starter-domicile' ); ?></h2>
			<p style="color:rgba(255,255,255,.7);font-size:1.05rem;line-height:1.6;margin:0 0 8px;"><?php esc_html_e( 'Votre theme est installe. Pour un site pret a l\'emploi en 1 clic (pages, menu, reglages), installez le plugin gratuit AG Starter Companion.', 'ag-starter-domicile' ); ?></p>
			<ul style="color:rgba(255,255,255,.6);font-size:.92rem;margin:12px 0 0;padding-left:18px;">
				<li><?php esc_html_e( 'Pages creees automatiquement (Accueil, Prestations, Zones d\'intervention, Temoignages, Devis, Contact)', 'ag-starter-domicile' ); ?></li>
				<li><?php esc_html_e( 'Menu principal configure et assigne', 'ag-starter-domicile' ); ?></li>
				<li><?php esc_html_e( 'Page d\'accueil et permaliens actives', 'ag-starter-domicile' ); ?></li>
				<li><?php esc_html_e( '100% gratuit, aucune connexion internet requise', 'ag-starter-domicile' ); ?></li>
			</ul>
		</div>
		<div style="text-align:center;">
			<a href="<?php echo esc_url( $search_url ); ?>" style="display:inline-block;background:#D4B45C;color:#0a0a0f;font-size:1.05rem;font-weight:700;padding:16px 32px;border-radius:8px;text-decoration:none;box-shadow:0 4px 16px rgba(212,180,92,.3);"><?php esc_html_e( 'Télécharger AG Starter Companion', 'ag-starter-domicile' ); ?></a>
			<p style="color:rgba(255,255,255,.4);font-size:.8rem;margin-top:10px;"><?php esc_html_e( 'Téléchargez le ZIP, puis Extensions → Ajouter → Téléverser', 'ag-starter-domicile' ); ?></p>
		</div>
	</div>
	<?php
}
add_action( 'admin_notices', 'ag_starter_domicile_companion_notice' );

function ag_starter_domicile_dashboard_widget() {
	if ( class_exists( 'AG_Starter_Companion' ) ) return;
	wp_add_dashboard_widget( 'ag_starter_welcome', esc_html__( '🚀 AG Starter Domicile — Configuration', 'ag-starter-domicile' ), 'ag_starter_domicile_dashboard_widget_render' );
...
function ag_starter_domicile_dashboard_widget_render() {
	$search_url = 'https://alliancegroupe-inc.com/wp-content/themes/alliance-groupe-theme/assets/downloads/ag-starter-companion.zip';
	?>
	<div style="text-align:center;padding:20px 0;">
		<p style="font-size:1.15rem;margin:0 0 16px;"><strong><?php esc_html_e( 'Votre theme est pret !', 'ag-starter-domicile' ); ?></strong></p>
		<p style="color:#666;margin:0 0 20px;"><?php esc_html_e( 'Installez le plugin gratuit AG Starter Companion pour creer automatiquement vos pages, votre menu et configurer votre site en 1 clic.', 'ag-starter-domicile' ); ?></p>
		<a href="<?php echo esc_url( $search_url ); ?>" class="button button-primary button-hero"><?php esc_html_e( 'Télécharger AG Starter Companion', 'ag-starter-domicile' ); ?></a>
		<p style="color:#999;font-size:.85rem;margin-top:12px;"><?php esc_html_e( 'Gratuit — 10 secondes — aucune inscription', 'ag-starter-domicile' ); ?></p>
	</div>
	<?php
}
add_action( 'wp_dashboard_setup', 'ag_starter_domicile_dashboard_widget' );
 ...
 */
function ag_domicile_resolve_cta_url( $raw ) {
	$raw = trim( (string) $raw );
	...
	if ( class_exists( 'ag_domicile_Reset' ) && method_exists( 'ag_domicile_Reset', 'install_pages_and_menu' ) ) {
		ag_domicile_Reset::install_pages_and_menu();
		$page = get_page_by_path( $slug );
		if ( $page ) return get_permalink( $page->ID );
	}
 ...
 */
if ( ! function_exists( 'ag_domicile_credit' ) ) :
	function ag_domicile_credit() {
		echo '<div class="ag-credit"><small>';
		echo '&copy; ' . esc_html( date( 'Y' ) ) . ' &mdash; ';
		printf(
			wp_kses( __( 'Site WordPress conçu par %s', 'ag-starter-domicile' ), array( 'a' => array( 'href' => array(), 'title' => array() ) ) ),
			'<a href="https://alliancegroupe-inc.com/wordpress-domicile" title="Site WordPress sur-mesure pour les services a la personne par Alliance Groupe">Alliance Groupe</a>'
		);
		echo ' &middot; <a href="https://alliancegroupe-inc.com/bureau-nantes" title="Agence web Alliance Groupe à Nantes">' . esc_html__( 'Agence Web Nantes', 'ag-starter-domicile' ) . '</a>';
		echo '</small></div>';
	}
endif;
