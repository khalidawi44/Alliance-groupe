...
$ag_has_preset = class_exists( 'ag_coach_Presets' ) && ag_coach_Presets::get_active_preset();
$ag_services   = $ag_has_preset ? ag_coach_Presets::get_active_services() : array();
$ag_how        = $ag_has_preset ? ag_coach_Presets::get_active_how() : array();
$ag_faq        = $ag_has_preset ? ag_coach_Presets::get_active_faq() : array();
$ag_stats      = $ag_has_preset ? ag_coach_Presets::get_active_stats() : array();
...
<main id="ag-main" class="<?php echo $ag_has_preset ? 'ag-main ag-main--premium' : 'ag-main'; ?>" role="main">
	...
	<?php else : ?>
		...
		$prest_fb_title = trim( ag_coach_opt( 'ag_coach_prestations_title_pre', 'Nos' ) . ' ' . ag_coach_opt( 'ag_coach_prestations_title_em', 'prestations' ) );
		list( $prest_title, $prest_lead ) = ag_coach_page_section_text( 'prestations', $prest_fb_title, ag_coach_opt( 'ag_coach_prestations_lead', '' ) );
		$zones_fb_title = trim( ag_coach_opt( 'ag_coach_zones_title_pre', 'Zones' ) . ' ' . ag_coach_opt( 'ag_coach_zones_title_em', 'd\'intervention' ) );
		list( $zones_title, $zones_lead ) = ag_coach_page_section_text( 'zones-intervention', $zones_fb_title, ag_coach_opt( 'ag_coach_zones_lead', '' ) );
		$real_fb_title = trim( ag_coach_opt( 'ag_coach_realisations_title_pre', 'Nos' ) . ' ' . ag_coach_opt( 'ag_coach_realisations_title_em', 'realisations' ) );
		list( $real_title, $real_lead ) = ag_coach_page_section_text( 'realisations', $real_fb_title, ag_coach_opt( 'ag_coach_realisations_lead', '' ) );
		?>
		<section class="ag-container" id="ag-services">
			<div class="ag-cards">
				<div class="ag-card">
					<h2><?php echo ag_coach_render_split_title( $prest_title ); ?></h2>
					<p><?php echo esc_html( $prest_lead ); ?></p>
				</div>
				<div class="ag-card">
					<h2><?php echo ag_coach_render_split_title( $zones_title ); ?></h2>
					<p><?php echo esc_html( $zones_lead ); ?></p>
				</div>
				<div class="ag-card">
					<h2><?php echo ag_coach_render_split_title( $real_title ); ?></h2>
	...
	<?php if ( $ag_has_preset ) :
		$t1 = ag_coach_Presets::get_testimonial( 1 );
		$t2 = ag_coach_Presets::get_testimonial( 2 );
		$t3 = ag_coach_Presets::get_testimonial( 3 );
		if ( $t1 || $t2 || $t3 ) : ?>
		<!-- Témoignages clients -->
	...
	if ( function_exists( 'ag_coach_render_promo_video' ) ) {
	...
	<?php if ( ! $ag_has_preset ) :
		...
		$about_fb_title = trim( ag_coach_opt( 'ag_coach_about_title_pre', 'Qui' ) . ' ' . ag_coach_opt( 'ag_coach_about_title_em', 'sommes-nous' ) );
		list( $about_title, $about_lead ) = ag_coach_page_section_text( 'qui-sommes-nous', $about_fb_title, ag_coach_opt( 'ag_coach_about_p1', '' ) );
		?>
		<section class="ag-info">
			<div class="ag-container">
				<h2><?php echo ag_coach_render_split_title( $about_title ); ?></h2>
				<p><?php echo esc_html( $about_lead ); ?></p>
				<p><?php echo esc_html( ag_coach_opt( 'ag_coach_about_p2', '' ) ); ?></p>
			</div>
		</section>
	<?php endif; ?>
