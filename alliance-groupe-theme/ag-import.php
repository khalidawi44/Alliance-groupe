...
function ag_run_import() {
    ...
    $cat_conseils = ag_get_or_create_category('Conseils Digital', 'conseils-digital');
    $cat_tech = ag_get_or_create_category('Tech & IA', 'tech-ia');
    $results[] = '✅ Catégories créées';
    ...
    foreach ($pages as $p) {
        ...
        if ($existing) {
            ...
            $results[] = '⏭️ Page "' . $p['title'] . '" existe déjà (ID: ' . $existing->ID . ')';
            continue;
        }
        $id = wp_insert_post([
            'post_title'   => $p['title'],
        ...
        if ($id && !is_wp_error($id)) {
            $page_ids[$p['slug']] = $id;
            $results[] = '✅ Page "' . $p['title'] . '" créée (ID: ' . $id . ')';
        }
    }
    ...
    $results[] = '✅ Réglages WordPress configurés (accueil statique, blog, permaliens)';
    ...
    if (!$menu_exists) {
        ...
        $menu_items = ['accueil' => 'Accueil', 'services' => 'Services', 'realisations' => 'Réalisations', 'a-propos' => 'À propos', 'blog' => 'Blog', 'contact' => 'Contact'];
        $position = 0;
        ...
        $results[] = '✅ Menu "Menu Principal" créé et assigné';
    } else {
        $results[] = '⏭️ Menu "Menu Principal" existe déjà';
    }

    // 5. Articles
    $articles = ag_get_articles();
    foreach ($articles as $a) {
        $existing = get_page_by_path($a['slug'], OBJECT, 'post');
        if ($existing) {
            $results[] = '⏭️ Article "' . $a['title'] . '" existe déjà';
            continue;
        }
        $cat_id = ($a['category'] === 'Tech & IA') ? $cat_tech : $cat_conseils;
        ...
        if ($post_id && !is_wp_error($post_id)) {
            ...
            $results[] = '✅ Article "' . $a['title'] . '" publié (ID: ' . $post_id . ')';
        }
    }
...
function ag_get_pages() {
    return [
        ['title' => 'Accueil', 'slug' => 'accueil', 'template' => 'templates/page-accueil.php'],
        ['title' => 'Services', 'slug' => 'services', 'template' => 'templates/page-services.php'],
        ['title' => 'Réalisations', 'slug' => 'realisations', 'template' => 'templates/page-realisations.php'],
        ['title' => 'À propos', 'slug' => 'a-propos', 'template' => 'templates/page-apropos.php'],
        ['title' => 'Contact', 'slug' => 'contact', 'template' => 'templates/page-contact.php'],
        ['title' => 'Blog', 'slug' => 'blog', 'template' => ''],
        ['title' => 'Création Web', 'slug' => 'service-creation-web', 'template' => 'templates/page-service-web.php'],
        ['title' => 'IA & Automatisation', 'slug' => 'service-ia', 'template' => 'templates/page-service-ia.php'],
        ['title' => 'SEO', 'slug' => 'service-seo', 'template' => 'templates/page-service-seo.php'],
        ['title' => 'Publicité Digitale', 'slug' => 'service-publicite', 'template' => 'templates/page-service-ads.php'],
        ['title' => 'Branding', 'slug' => 'service-branding', 'template' => 'templates/page-service-brand.php'],
        ['title' => 'Conseil Stratégique', 'slug' => 'service-conseil', 'template' => 'templates/page-service-conseil.php'],
    ];
}

/* ── Articles ─────────────────────────────────────────────────── */
function ag_get_articles() {
    return [
        // ── Article 1 ──
        [
            'title'    => 'Pourquoi 80% des PME sans site web perdent des clients chaque jour',
            'slug'     => 'pme-sans-site-web-perdent-clients',
            'category' => 'Conseils Digital',
            'excerpt'  => 'Chaque jour sans site web professionnel, votre entreprise perd des clients au profit de vos concurrents. Découvrez pourquoi et comment y remédier.',
            'tags'     => 'site web, PME, clients, visibilité',
            'content'  => '<p>En France, <strong>80% des consommateurs recherchent une entreprise en ligne avant de la contacter</strong>. Si votre entreprise n\'a pas de site web — ou pire, un site obsolète — vous êtes tout simplement invisible pour la majorité de vos clients potentiels.</p>
...
<blockquote>Premier diagnostic 100% gratuit — Sans engagement — Réponse sous 24h</blockquote>',
        ...
        // ── Article 2 ──
        [
            'title'    => 'Combien vous coûte réellement l\'absence de présence digitale ?',
            'slug'     => 'cout-absence-presence-digitale',
            'category' => 'Conseils Digital',
            'excerpt'  => 'L\'absence de présence digitale coûte en moyenne 120 000€ par an aux PME françaises. Calculez combien vous perdez et comment inverser la tendance.',
            'tags'     => 'présence digitale, ROI, coûts, stratégie',
            'content'  => '<p>Vous pensez économiser en n\'investissant pas dans le digital ? En réalité, <strong>l\'absence de présence digitale est la dépense la plus coûteuse que vous ne voyez jamais sur vos relevés bancaires.</strong></p>
...
<blockquote>Audit de votre manque à gagner 100% gratuit — Sans engagement — Réponse sous 24h</blockquote>',
        ...
        // ── Article 3 ──
        [
            'title'    => '5 signes que votre site web fait fuir vos clients (et comment y remédier)',
            'slug'     => 'signes-site-web-fait-fuir-clients',
            'category' => 'Conseils Digital',
            'excerpt'  => 'Votre site est lent, pas responsive ou mal structuré ? Voici les 5 signaux d\'alarme qui font fuir vos visiteurs et comment les corriger rapidement.',
            'tags'     => 'site web, UX, conversion, optimisation',
            'content'  => '<p>Vous avez un site web, mais il ne génère aucun contact ? <strong>Le problème n\'est peut-être pas le manque de visiteurs — c\'est que votre site les fait fuir.</strong></p>
...
<blockquote>Diagnostic complet de votre site gratuit — Sans engagement — Résultats en 48h</blockquote>',
        ...
        // ── Article 4 ──
        [
            'title'    => 'Comment l\'IA révolutionne la génération de leads en 2025',
            'slug'     => 'ia-revolution-generation-leads-2025',
            'category' => 'Tech & IA',
            'excerpt'  => 'L\'intelligence artificielle transforme radicalement la façon dont les entreprises génèrent des leads. Chatbots, automatisation, personnalisation : découvrez comment en profiter.',
            'tags'     => 'IA, leads, automatisation, chatbot',
            'content'  => '<p>L\'intelligence artificielle n\'est plus de la science-fiction. En 2025, <strong>les entreprises qui utilisent l\'IA pour leur génération de leads surpassent leurs concurrents de 50% en taux de conversion.</strong></p>
...
<blockquote>Consultation IA gratuite — On vous montre concrètement ce qui est possible — Sans engagement</blockquote>',
        ...
        // ── Article 5 ──
        [
            'title'    => 'SEO local : pourquoi vos concurrents vous volent vos clients sur Google',
            'slug'     => 'seo-local-concurrents-volent-clients-google',
            'category' => 'Conseils Digital',
            'excerpt'  => 'Si vous n\'apparaissez pas dans le top 3 de Google pour vos recherches locales, vos concurrents récupèrent 75% de vos clients potentiels. Voici comment réagir.',
            'tags'     => 'SEO, Google, local, concurrents',
            'content'  => '<p>Tapez votre métier + votre ville sur Google. Vous apparaissez ? Si la réponse est non — ou si vous êtes en dessous de la 3ème position — <strong>vos concurrents captent 75% des clients qui vous cherchent</strong>.</p>
...
<blockquote>Audit SEO local offert — On vous dit exactement où vous en êtes et comment passer en Top 3</blockquote>',
        ...
        // ── Article 6 ──
        [
            'title'    => 'Le vrai coût d\'un commercial vs un site web optimisé : la comparaison qui fait mal',
            'slug'     => 'cout-commercial-vs-site-web-optimise',
            'category' => 'Conseils Digital',
            'excerpt'  => 'Un commercial coûte 45 000€/an minimum. Un site web optimisé génère des leads 24/7 pour une fraction du prix. Voici la comparaison détaillée.',
            'tags'     => 'commercial, site web, ROI, leads, coûts',
            'content'  => '<p>Recruter un commercial ou investir dans un site web performant ? <strong>Quand on pose les chiffres sur la table, la réponse est sans appel.</strong></p>
...
<blockquote>Simulation ROI personnalisée offerte — Voyez concrètement combien vous pouvez économiser</blockquote>',
        ...
        // ── Article 7 ──
        [
            'title'    => 'Étude de cas : comment un paysagiste a multiplié ses devis par 4 en 4 mois',
            'slug'     => 'etude-cas-paysagiste-multiplie-devis-par-4',
            'category' => 'Conseils Digital',
            'excerpt'  => 'L.A Environnement est passé de 3 devis par mois à 15 grâce à une stratégie digitale complète. Voici exactement comment nous avons fait.',
            'tags'     => 'étude de cas, paysagiste, devis, stratégie digitale',
            'content'  => '<p>Quand L.A Environnement nous a contactés, la situation était classique : <strong>un excellent artisan, totalement invisible en ligne.</strong> 4 mois plus tard, il croulait sous les demandes de devis.</p>
...
<blockquote>Stratégie personnalisée gratuite — On vous montre exactement ce qu\'on ferait pour votre entreprise</blockquote>',
        ...
        // ── Article 8 ──
        [
            'title'    => 'Les 7 erreurs fatales qui tuent votre visibilité en ligne',
            'slug'     => 'erreurs-fatales-tuent-visibilite-en-ligne',
            'category' => 'Conseils Digital',
            'excerpt'  => 'De l\'absence de stratégie SEO au site non responsive, ces 7 erreurs courantes détruisent silencieusement votre visibilité digitale. Corrigez-les avant qu\'il ne soit trop tard.',
            'tags'     => 'erreurs, visibilité, SEO, stratégie',
            'content'  => '<p>Votre entreprise est peut-être excellente. Vos services peut-être irréprochables. Mais si personne ne vous trouve en ligne, <strong>tout ça ne sert à rien.</strong></p>
...
<blockquote>Audit de vos 7 erreurs offert — Rapport détaillé en 48h — Sans engagement</blockquote>',
        ...
        // ── Article 9 ──
        [
            'title'    => 'Automatisation : comment gagner 15 heures par semaine grâce à l\'IA',
            'slug'     => 'automatisation-gagner-15-heures-semaine-ia',
            'category' => 'Tech & IA',
            'excerpt'  => 'L\'automatisation par l\'IA peut vous faire gagner jusqu\'à 15 heures par semaine sur les tâches répétitives. Découvrez les outils et stratégies qui changent la donne.',
            'tags'     => 'automatisation, IA, productivité, outils',
            'content'  => '<p>Combien de temps passez-vous chaque semaine à répondre aux mêmes emails, relancer des prospects, générer des factures ou publier sur les réseaux sociaux ? <strong>La majorité des entrepreneurs perdent 15 à 20 heures par semaine sur des tâches répétitives que l\'IA peut automatiser.</strong></p>
...
<blockquote>Audit automatisation gratuit — On identifie vos heures perdues et comment les récupérer</blockquote>',
        ...
        // ── Article 10 ──
        [
            'title'    => 'Votre site web ne génère aucun lead ? Voici les 6 raisons (et les solutions)',
            'slug'     => 'site-web-ne-genere-aucun-lead-raisons-solutions',
            'category' => 'Conseils Digital',
            'excerpt'  => 'Si votre site web est une brochure en ligne plutôt qu\'une machine à leads, c\'est probablement pour l\'une de ces 6 raisons. Diagnostic et solutions concrètes.',
            'tags'     => 'leads, site web, conversion, diagnostic',
            'content'  => '<p>Vous avez investi dans un site web. Il est en ligne. Mais le téléphone ne sonne pas davantage. Le formulaire de contact reste vide. <strong>Votre site est une brochure numérique, pas une machine à leads.</strong></p>
