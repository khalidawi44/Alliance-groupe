# AGENTS.md — Infrastructure Alliance Groupe (réutilisable multi-sites)
...
- Sur Codex web, **chaque conversation démarre dans un conteneur neuf** : seul ce qui est **commité + poussé** survit.
- **Hook `SessionStart`** (`.Codex/hooks/session-start.sh`, enregistré dans `.Codex/settings.json`) : à chaque nouvelle session il injecte automatiquement l'état réel (branche, derniers commits, travail non commité, version active, focus templates, en-tête `HANDOFF.md`). C'est ce qui **relie les conversations** entre elles.
...
- Dév sur la branche de travail courante (ex. `Codex/infrastructure-md-review-6R3D2` ; vérifier `git branch --show-current`), puis **merge ff-only dans `main`** + push.
